...
impl Transcript {
    ...
    pub(super) fn row(&self, index: usize) -> Option<&str> {
    ...
    pub(super) fn ceiling(&self, height: usize) -> usize {
        self.rows().saturating_sub(height.max(1))
    }

    pub(super) fn text(&mut self, character: char) {
        match character {
            '\n' => self.newline(),
    ...
    pub(super) fn escape(&mut self, character: char) {
    ...
    pub(super) fn csi(&mut self, character: char) {
    ...
    pub(super) fn terminated(&mut self, character: char) {
        self.scan = match character {
            '\u{7}' => Scan::Text,
