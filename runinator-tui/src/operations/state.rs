...
pub(super) struct State {
    ...
    pub(super) input: Option<InputMode>,
    pub(super) workflows: Vec<WorkflowDefinition>,
    pub(super) runs: Vec<WorkflowRun>,
    pub(super) groups: Vec<ProvisionedGroup>,
    pub(super) replicas: Option<ReplicaListResponse>,
    pub(super) logs: Vec<RuntimeLogRecord>,
    pub(super) logs_paused: bool,
    pub(super) log_level_filter: Option<String>,
    pub(super) status: String,
    pub(super) refreshed_at: Option<Instant>,
    pub(super) local_path: Option<PathBuf>,
    pub(super) local_snapshot: Option<LocalDashboardSnapshot>,
    pub(super) detail: Option<(String, Vec<String>)>,
    pub(super) detail_scroll: u16,
}

impl State {
    pub(super) fn new(client: Client, server: String, local_path: Option<PathBuf>) -> Self {
        Self {
            client,
            server,
            tab: 0,
    ...
    pub(super) async fn handle_input(&mut self, key: KeyEvent) -> Result<bool, Error> {
    ...
    pub(super) fn refresh_local(&mut self) {
    ...
    pub(super) async fn launch_json(&mut self, source: &str) {
        let Some(workflow) = self.filtered_workflows().get(self.selected).copied() else {
            return;
        };
    ...
    pub(super) async fn launch_form(&mut self, form: LaunchForm) {
    ...
    pub(super) async fn run_action(&mut self, action: &str) {
    ...
    pub(super) async fn replay(&mut self) {
        let Some(run) = self.filtered_runs().get(self.selected).copied() else {
            return;
        };
    ...
    pub(super) async fn replay_from(&mut self, run_id: uuid::Uuid, from_step_id: Option<String>) {
        let plan = match self
            .client
    ...
    pub(super) async fn scale(&mut self, direction: i32) {
    ...
    pub(super) async fn confirm(&mut self, action: ConfirmAction) {
        match action {
    ...
    pub(super) async fn inspect_run(&mut self) {
    ...
    pub(super) fn filtered_workflows(&self) -> Vec<&WorkflowDefinition> {
        self.workflows
            .iter()
    ...
    pub(super) fn filtered_runs(&self) -> Vec<&WorkflowRun> {
        self.runs
            .iter()
            .filter(|item| {
                matches_query(
    ...
    pub(super) fn filtered_groups(&self) -> Vec<&ProvisionedGroup> {
        self.groups
            .iter()
            .filter(|item| {
                matches_query(
    ...
    pub(super) fn draw(&self, frame: &mut ratatui::Frame) {
    ...
    pub(super) fn draw_body(&self, frame: &mut ratatui::Frame, area: ratatui::layout::Rect) {
    ...
    pub(super) fn overview_lines(&self) -> Vec<Line<'static>> {
        let active = self
            .runs
            .iter()
    ...
    pub(super) fn organization_label(&self) -> String {
    ...
    pub(super) fn component_lines(&self) -> Vec<Line<'static>> {
        let mut lines = self
            .local_snapshot
            .as_ref()
            .into_iter()
    ...
    pub(super) fn workflow_lines(&self) -> Vec<Line<'static>> {
