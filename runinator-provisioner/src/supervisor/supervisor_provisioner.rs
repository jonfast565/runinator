...
pub struct SupervisorProvisioner {
    ...
    pub(super) templates: BTreeMap<&'static str, SupervisorNodeTemplate>,
}
...
impl SupervisorProvisioner {
    ...
    pub(super) fn template(
        &self,
        kind: ReplicaKind,
    ) -> Result<&SupervisorNodeTemplate, SendableError> {
        self.templates
            .get(kind.as_str())
    ...
    pub(super) fn current_nodes(&self, prefix: &str) -> Result<Vec<(String, bool)>, SendableError> {
    ...
    pub(super) fn enqueue(&self, command: &ControlCommand) -> Result<(), SendableError> {
