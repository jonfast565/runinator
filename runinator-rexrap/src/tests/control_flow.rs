...
fn bound_for_collects_the_loop_results_array() {
    let src = r#"
        workflow "CollectedLoop" v1 {
            ...
            do {
                let results = for item in params.items {
                    let echoed = console.run(command: item)
                }
...
fn typed_compute_output_hint_validates_loop_items() {
    let src = r#"
        workflow "TypedComputeLoop" v1 {
            ...
            do {
                let impact: { lambdas: string[] } = compute {
...
fn action_node_parameters_are_not_dropped() {
    ...
    let mut def = compile(r#"workflow "Params" { do { console.run(command: "probe") } }"#);
    let action = def
        .definition
        .nodes
        .iter_mut()
