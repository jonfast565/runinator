...
async fn process_one_task(pool: &SqlitePool, libraries: &HashMap<String, String>, task: &ScheduledTask, task_handles: Arc<Mutex<HashMap<Uuid, tokio::task::JoinHandle<()>>>>) {
...
async fn scheduler_loop(pool: &SqlitePool, libraries: &HashMap<String, String>, notify: Arc<Notify>, config: &Config) {
    let task_handles: Arc<Mutex<HashMap<Uuid, tokio::task::JoinHandle<()>>>> = Arc::new(Mutex::new(HashMap::new()));
    loop {
        ...
        tokio::select! {
            _ = notified => {
                ...
                let handles = &map_clone.lock().unwrap();
            ...
            _ = tokio::time::sleep(tokio::time::Duration::from_secs(1)) => {
                ...
                for task in tasks {
                    process_one_task(pool, &libraries, &task, task_handles.clone()).await;
                }
            }
        }
    }
