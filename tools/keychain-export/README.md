...
`cargo run -p runinator-desktop-agent` stages the helper on macOS. It builds
and optionally signs it only when the release binary is absent; later runs reuse
that copy. A profile command using `keychain-export` resolves to the staged
copy, so a development agent does not depend on your shell `PATH`.
