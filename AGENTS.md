...
- `runinator-runtime` owns graph interpretation and durable state transitions through its host boundary. `runinator-engine` drives it and publishes `ActionCommand` values through `runinator-broker` by draining the durable `workflow_action_dispatches` outbox. The engine normally runs in-process with the web service, but may instead run in `runinator-engine-worker`. The waker never publishes `ActionCommand`s.
