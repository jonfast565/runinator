...
Every documented HTTP operation also exports an `x-runinator-control-surface` classification (`operator`, `headless`, `data_plane`, or `bootstrap`). Route/document parity and classification tests make newly added server capabilities visible to control-surface audits instead of allowing uncategorized endpoints to accumulate silently.
