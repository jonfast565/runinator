...
Inbound orchestration adapters are deliberately separate from outbound providers. `runinator-adapter-host` runs adapter code on loopback only: authoring handlers use it for kind discovery, draft validation, webhook verification, and safe tests, while the engine uses it for durable polling. It ships as a sidecar of the web-service and standalone-engine pods, not as a network Service. The adapter client, shared request contract, and SDK keep both call paths on one authenticated protocol. Adapter ABI v2 requires every dynamic adapter to export validation alongside metadata, webhook handling, and polling; v1 libraries must be rebuilt with the current SDK. Kind metadata owns webhook and polling fields, identity keys, authentication choices, and execution-profile requirements so Command Center can render adapter-specific forms without provider branches.

Every documented HTTP operation also exports an `x-runinator-control-surface` classification (`operator`, `headless`, `data_plane`, or `bootstrap`). Operator operations name an `x-runinator-capability`; other operations explain their fixed boundary. Route/document parity and the [Command Center coverage contract](control-surface-coverage.md) make newly added server capabilities visible to control-surface audits instead of allowing uncategorized endpoints to accumulate silently.
