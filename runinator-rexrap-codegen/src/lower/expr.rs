// lowers rexrap expressions and conditions into the json forms the runtime understands:
...
impl Lowerer {
    ...
    pub(super) fn lower_expr(&self, expr: &Expr) -> Result<Value, RexRapError> {
        match &expr.kind {
            ...
            ExprKind::Object(entries) => {
                ...
                let flat = runinator_rexrap_sema::desugar::flatten_entries(entries, &self.aliases)?;
                let mut map = Map::new();
            ...
            ExprKind::Call {
                ...
                let positional = self
                    ...
                    .map_err(|err| RexRapError::semantic(expr.span, err))?;
                let args = positional
                    .iter()
            ...
            ExprKind::Spread(name) => {
                Err(RexRapError::lower(format!("unexpanded spread '...{name}'")))
            }
        }
    }

    fn lower_file_include(&self, expr: &Expr, include_path: &str) -> Result<Value, RexRapError> {
        let Some(source_dir) = &self.source_dir else {
            return Err(RexRapError::semantic(
                expr.span,
                "file() requires a source directory; compile from a .rexrap file or pack source",
            ));
        };
    ...
    fn lower_dir_include(
        ...
        let Some(source_dir) = &self.source_dir else {
            return Err(RexRapError::semantic(
                expr.span,
                "dir() requires a source directory; compile from a .rexrap file or pack source",
            ));
        };
        let relative = Path::new(include_path);
        if relative.as_os_str().is_empty() {
        ...
        let entries =
            runinator_rexrap_syntax::includes::dir_relative_files(&base, recursive, max_depth)
                .map_err(|err| {
                    RexRapError::semantic(
                        expr.span,
    ...
    fn lower_string(&self, parts: &[StrPart]) -> Result<Value, RexRapError> {
    ...
    fn wrap_array(&self, key: &str, parts: &[Expr]) -> Result<Value, RexRapError> {
        let items = parts
            .iter()
    ...
    fn wrap_unary(&self, key: &str, inner: &Expr) -> Result<Value, RexRapError> {
        Ok(single_key(key, self.lower_expr(inner)?))
    }

    fn lower_path(&self, segs: &[PathSeg]) -> Result<Value, RexRapError> {
        let head = match segs.first() {
            Some(PathSeg::Key(name)) => name.clone(),
            _ => return Err(RexRapError::lower("path must start with an identifier")),
        };
        let rest = &segs[1..];
    ...
    pub(super) fn lower_cond(&self, cond: &Cond) -> Result<Value, RexRapError> {
        match &cond.kind {
            CondKind::All(parts) => {
                let items = parts
                    .iter()
...
fn lower_secret(rest: &[PathSeg]) -> Result<Value, RexRapError> {
    let parts = rest
        .iter()
        .map(|seg| match seg {
