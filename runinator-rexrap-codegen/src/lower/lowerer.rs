...
pub(super) struct Lowerer {
    ...
    pub(super) used_ids: HashSet<String>,
    ...
    pub(super) parallel_branches: Map,
    ...
    pub(super) compute_locals: std::cell::RefCell<HashSet<String>>,
    // published packaged-function exports a `functions.<pkg>.<export>(...)` call may bind to.
    pub(super) functions: Vec<runinator_models::functions::FunctionCatalogEntry>,
    // resolved `type <Name>` declarations, consulted when lowering named type references.
    pub(super) named_types:
        std::collections::BTreeMap<String, runinator_models::types::RuninatorType>,
    ...
    pub(super) lowered_functions: Vec<Value>,
    ...
    pub(super) registry: runinator_rexrap_sema::registry::FunctionRegistry,
    // provider actions available at compile time, used to generate action output type hints.
    pub(super) provider_actions: std::collections::HashMap<(String, String), ActionMetadata>,
    ...
    pub(super) provider_metadata: Vec<runinator_models::providers::ProviderMetadata>,
}
...
impl Lowerer {
    ...
    pub(super) fn apply_function_binding(
        &self,
        action: &ActionStmt,
        action_obj: &mut Map,
    ...
    pub(super) fn resolve_type_decls(&mut self, decls: &[TypeDecl]) -> Result<(), RexRapError> {
    ...
    pub(super) fn lower_notifications(
        &self,
        policies: &[NotifyDecl],
    ) -> Result<Vec<Value>, RexRapError> {
    ...
    pub(super) fn lower_watches(&self, watches: &[WatchDecl]) -> Result<Vec<Value>, RexRapError> {
    ...
    pub(super) fn record_generated_type_hint(
        &mut self,
        id: &str,
        action: &ActionStmt,
    ) -> Result<(), RexRapError> {
    ...
    pub(super) fn entry_id_for(&mut self, stmt: &Stmt) -> Result<LowerEntry, RexRapError> {
    ...
    pub(super) fn lower_value_collector(
        &mut self,
        stmt: &Stmt,
        id: &str,
        next: &str,
    ...
    pub(super) fn lower_action_object(
        &mut self,
        action: &ActionStmt,
    ...
    pub(super) fn lower_circuit_breaker(
        &mut self,
    ...
    pub(super) fn lower_event_source(
        &mut self,
        es: &EventSourceStmt,
        stmt: &Stmt,
        id: &str,
    ...
    pub(super) fn apply_modifier_fields(
        &self,
    ...
    pub(crate) fn apply_annotations(&self, fields: &mut Vec<(&'static str, Value)>, stmt: &Stmt) {
    ...
    pub(super) fn block_cont(&self, clause: &TransitionClause, cont: &str) -> String {
    ...
    pub(super) fn target_id(&self, target: &Target) -> String {
        match target {
    ...
    pub(super) fn push(&mut self, node: Value) {
    ...
    pub(super) fn claim(&mut self, id: &str) -> Result<String, RexRapError> {
    ...
    pub(super) fn fresh_resume(&mut self) -> String {
        loop {
