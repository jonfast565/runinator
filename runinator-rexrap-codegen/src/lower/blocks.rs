// expands rexrap control blocks into the matching runinator control nodes. each builder
...
impl Lowerer {
    ...
    pub(super) fn lower_for(
        ...
        if let Some(var_type) = &for_stmt.var_type {
            vars.insert(
                "item_type".into(),
                Value::String(runinator_rexrap_syntax::format::format_type(var_type)),
            );
        }
        if let Some(index_var) = &for_stmt.index_var {
    ...
    fn lower_switch_cases(
        ...
        for arm in &match_stmt.arms {
            ...
            if let Some(when) = &arm.when {
                ...
                return Err(RexRapError::lower("match arm needs a value or when clause"));
            }
    ...
    fn lower_toggle(
        ...
        let value = self.lower_expr(&match_stmt.subject)?;
        let arm_target = |lowerer: &mut Self, want: bool| -> Result<Value, RexRapError> {
            let arm = match_stmt
                .arms
                .iter()
                .find(|arm| arm.toggle == Some(want))
                .ok_or_else(|| RexRapError::lower("toggle requires an `on` and an `off` arm"))?;
    ...
    fn lower_percentage(
        ...
        for arm in &match_stmt.arms {
            let weight = arm
                .weight
                .ok_or_else(|| RexRapError::lower("split arm needs a percentage weight"))?;
            let entry = self.lower_block(&arm.body, cont)?;
