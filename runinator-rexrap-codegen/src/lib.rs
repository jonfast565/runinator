// runinator-rexrap-codegen: the mapping between the rexrap ast and the runtime json workflow model,
// in both directions. `lower` turns an analyzed ast into a `WorkflowDefinition`; `decompile`
// turns a `WorkflowDefinition` back into rexrap source text.
//
...
// so that contract has one owner. the round-trip assertions themselves are in `runinator-rexrap`,
