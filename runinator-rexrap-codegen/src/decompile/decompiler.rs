...
pub(crate) struct Decompiler<'a> {
    pub(super) nodes: HashMap<String, &'a WorkflowNode>,
    pub(super) end_ids: HashSet<String>,
    pub(super) fail_ids: HashSet<String>,
    // surface every implicit construct (ids, edges, defaults) instead of the terse form.
    pub(super) explicit: bool,
    pub(super) loop_vars: Vec<(String, String)>,
    ...
    pub(super) worklist: VecDeque<String>,
...
impl<'a> Decompiler<'a> {
    pub(super) fn emit_resource_imports(&mut self) {
    ...
    pub(super) fn resource_alias(&self, kind: &str, path: &str) -> Option<&str> {
    ...
    pub(super) fn loop_var(&self, node_id: &str) -> Option<String> {
        self.loop_vars
            .iter()
            .rev()
    ...
    pub(super) fn emit_params(&mut self, input_type: &RuninatorType) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_functions(&mut self, functions: &[FnEntry]) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_triggers(&mut self, triggers: &[Value]) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_notifications(&mut self, policies: &[Value]) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_watches(&mut self, watches: &[Value]) -> Result<(), RexRapError> {
        if watches.is_empty() {
            return Ok(());
        }
    ...
    pub(super) fn interrupt_regions(
        &mut self,
        graph: &WorkflowGraph,
        metadata: &[Value],
    ...
    pub(super) fn emit_ingress(&mut self, value: Option<&Value>) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_alias_decls(&mut self) -> Result<(), RexRapError> {
    ...
    pub(super) fn emit_region(&mut self, cur: &str, stop: Option<&str>) -> Result<(), RexRapError> {
    ...
    pub(super) fn separate_block_statement(
        &mut self,
        start: usize,
        prev_multiline: &mut Option<bool>,
    ) {
    ...
    pub(super) fn is_terminal(&self, id: &str) -> bool {
    ...
    pub(super) fn is_join(&self, id: &str) -> bool {
        self.nodes
            .get(id)
    ...
    pub(super) fn block_id_prefix(&self, node: &WorkflowNode) -> String {
        self.annotation_prefix(node, self.should_emit_control_id(node))
    }

    pub(super) fn should_emit_control_id(&self, node: &WorkflowNode) -> bool {
        self.explicit || self.control_ids.contains(&node.id) || !is_generated_control_id(node)
    }

    pub(super) fn annotation_prefix(&self, node: &WorkflowNode, include_id: bool) -> String {
    ...
    pub(super) fn target_label(&self, id: &str) -> String {
    ...
    pub(super) fn defer(&mut self, id: &str) {
    ...
    pub(super) fn close_block_route(&mut self, closing: &str, head: &str, label: &str) {
        self.line(closing);
        self.line("routes {");
        self.indent += 1;
    ...
    pub(super) fn statement_text(
        &self,
        node: &WorkflowNode,
    ) -> Result<(String, bool), RexRapError> {
        match &node.kind {
            WorkflowNodeKind::Action => {
    ...
    pub(super) fn compute_text(&self, program: &[Value]) -> Result<String, RexRapError> {
        let base = self.indent;
    ...
    pub(super) fn render_compute_lines(
        &self,
        out: &mut String,
        program: &[Value],
        indent: usize,
    ...
    pub(super) fn foreign_compute_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let action = node
            .action
            .as_ref()
    ...
    pub(super) fn call_args(&self, parts: &[String], base: usize) -> String {
        if parts.is_empty() {
            return "()".to_string();
        }
    ...
    pub(super) fn parts_object(&self, parts: &[String], base: usize) -> String {
        if parts.is_empty() {
            return "{}".to_string();
        }
        let inner = "    ".repeat(base + 1);
    ...
    /// them onto it.
    pub(super) fn modifier_prefix(&self, modifiers: &[String]) -> String {
    ...
    pub(super) fn action_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let action = node
            .action
            .as_ref()
    ...
    pub(super) fn node_attributes(&self, node: &WorkflowNode) -> Result<Vec<String>, RexRapError> {
    ...
    pub(super) fn call_timeout_modifier(
        &self,
    ...
    pub(super) fn subflow_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn wait_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn output_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let items = node
            .parameters
            .get("items")
            .and_then(Value::as_array)
    ...
    pub(super) fn fail_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn resume_text(&self, node: &WorkflowNode) -> String {
    ...
    pub(super) fn input_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn approval_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let prompt = node
            .parameters
            .get("prompt")
            .cloned()
    ...
    pub(super) fn gate_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let kind = node
            .parameters
            .get("kind")
            .and_then(Value::as_str)
    ...
    pub(super) fn signal_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn assert_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let assertions = node
            .parameters
            .get("assertions")
            .and_then(Value::as_array)
    ...
    pub(super) fn transform_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let bindings = node
            .parameters
            .get("bindings")
            .cloned()
    ...
    pub(super) fn audit_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let action = node
            .parameters
            .get("action")
            .cloned()
    ...
    pub(super) fn checkpoint_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn mutex_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn find_mutex_release(
        &self,
        acquire: &WorkflowNode,
    ...
    pub(super) fn throttle_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn cooldown_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn await_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn debounce_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn collect_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn barrier_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn circuit_breaker_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let name = node
            .parameters
            .get("name")
            .and_then(Value::as_str)
    ...
    pub(super) fn event_source_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
        let event_type = node
            .parameters
            .get("event_type")
            .and_then(Value::as_str)
    ...
    pub(super) fn config_text(&self, node: &WorkflowNode) -> Result<String, RexRapError> {
    ...
    pub(super) fn fresh_var(&self) -> String {
    ...
    pub(super) fn find_merge(&self, starts: &[String]) -> Option<String> {
        if starts.is_empty() {
            return None;
        }
    ...
    pub(super) fn reachable(&self, start: &str) -> HashMap<String, usize> {
    ...
    pub(super) fn out_edges(&self, node: &WorkflowNode) -> Vec<String> {
