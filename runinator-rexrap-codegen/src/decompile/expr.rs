// inverts the json expression/condition forms back into rexrap surface text. this is the
...
impl Decompiler<'_> {
    pub(super) fn expr(&self, value: &Value) -> Result<String, RexRapError> {
        match value {
    ...
    pub(super) fn expr_multiline(&self, value: &Value, base: usize) -> Result<String, RexRapError> {
        match value {
            Value::Object(map) => {
                if let Some(special) = self.special_form(map)? {
                    return Ok(special);
    ...
    fn special_form(
        ...
        if let Some(name) = map.get("$call").and_then(Value::as_str) {
            ...
            let callee = runinator_compute::qualified_intrinsic_name(name)
        ...
        if map.len() == 1 {
            if let Some(spec) = map.get("$lambda").and_then(Value::as_object) {
                ...
                let body = spec
                    .get("body")
                    .ok_or_else(|| RexRapError::Decompile("$lambda missing body".into()))?;
    ...
    pub(super) fn render_segs(&self, segs: &[Value]) -> Result<String, RexRapError> {
    ...
    pub(super) fn render_seg_parts(&self, segs: &[Value]) -> Result<Vec<String>, RexRapError> {
        ...
        for seg in segs {
            ...
            let key = seg.get("key").and_then(Value::as_str).ok_or_else(|| {
                RexRapError::Decompile("spread recipe segment missing key".into())
            })?;
            let value = seg.get("value").ok_or_else(|| {
                RexRapError::Decompile("spread recipe segment missing value".into())
            })?;
    ...
    fn render_recipe_value(&self, value: &Value) -> Result<String, RexRapError> {
        ...
        Err(RexRapError::Decompile("invalid spread recipe value".into()))
    }

    fn join_binary(&self, items: &[Value], sep: &str) -> Result<String, RexRapError> {
        let parts = items
            .iter()
            .map(|item| self.expr(item))
            .collect::<Result<Vec<_>, _>>()?;
    ...
    fn arith(&self, items: &[Value], sep: &str, prec: u8) -> Result<String, RexRapError> {
    ...
    // below `min_prec`.
    fn arith_operand(&self, value: &Value, min_prec: u8) -> Result<String, RexRapError> {
    ...
    fn access(&self, base: &Value, key: &Value) -> Result<Option<String>, RexRapError> {
    ...
    fn reference(&self, reference: &Value) -> Result<String, RexRapError> {
        let object = reference
            .as_object()
            .ok_or_else(|| RexRapError::Decompile("invalid $ref".into()))?;
        ...
        if let (Some(node), Some(output)) = (object.get("node"), object.get("output")) {
            let node_id = node
                .as_str()
                .ok_or_else(|| RexRapError::Decompile("invalid $ref node".into()))?;
        ...
        Err(RexRapError::Decompile("unrecognized $ref".into()))
    }
    ...
    pub(super) fn cond(&self, value: &Value) -> Result<String, RexRapError> {
        let object = value
            .as_object()
            .ok_or_else(|| RexRapError::Decompile("condition must be an object".into()))?;
        ...
        let left = object
            ...
            .ok_or_else(|| RexRapError::Decompile("condition missing value".into()))?;
        ...
        Err(RexRapError::Decompile("unrecognized condition".into()))
    }

    fn join_cond(&self, items: &[Value], sep: &str) -> Result<String, RexRapError> {
        let parts = items
            .iter()
            .map(|item| {
...
/// render a RuninatorType as a rexrap type expression.
