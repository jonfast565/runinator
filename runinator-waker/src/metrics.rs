...
pub(crate) fn wake_received(lead_ms: f64) {
    runinator_tui::counter("waker", "wakes received", 1);
    metrics().wakes_received.add(1, &[]);
    metrics().wake_lead_ms.record(lead_ms, &[]);
}

pub(crate) fn wake_due_lag(lag_ms: f64) {
    runinator_tui::gauge("waker", "due lag (ms)", lag_ms.max(0.0) as i64);
...
pub(crate) fn drive_failed() {
    runinator_tui::counter("waker", "settle failures", 1);
...
pub(crate) fn broker_heartbeat() {
    runinator_tui::counter("waker", "broker heartbeats", 1);
...
pub(crate) fn broker_heartbeat_failed() {
    runinator_tui::counter("waker", "heartbeat failures", 1);
