...
async fn dispatch_compensation<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
    origin: &str,
    ...
    let command = ActionCommand {
        ...
        // must not share the forward action's key. declaring one on the compensating operation itself
...
async fn persist_frame<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
...
async fn finalize_fail<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
) -> Result<ReadyNodeDisposition, SendableError> {
