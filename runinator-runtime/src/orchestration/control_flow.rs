    ...
    pub(super) async fn process<T: RuntimeStore>(
        ...
        let items = match existing_frame {
            ...
            None => match runinator_workflows::parse_loop_parameters(&ctx.node.id, &parameters) {
                ...
                Err(err) => {
                    return super::execution::complete(block_node(ctx, &err.to_string()).await);
                }
            },
        };
...
async fn enqueue_branch<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
    cursor_id: Uuid,
    branch: &str,
    reason: &str,
...
/// speculative output before this operation ran, a "what if" fork can never satisfy a real join.
...
impl JoinOp {
    pub(super) async fn process<T: RuntimeStore>(
        ...
        {
            return super::execution::complete(
                time_out(ctx, node_run, "Join node timed out").await,
            );
        }
...
impl RaceOp {
    pub(super) async fn process<T: RuntimeStore>(
        ...
        if node_run.status == WorkflowStatus::Running && timed_out(ctx.timing(), &node_run) {
            return super::execution::complete(
                time_out(ctx, &node_run, "Race node timed out").await,
            );
        }
        let branches = params
            .branches
        ...
        if branches.is_empty() {
            return super::execution::complete(block_node(ctx, "Race node has no branches").await);
        }
...
impl TryOp {
    pub(super) async fn process<T: RuntimeStore>(
        ...
        if node_run.status == WorkflowStatus::Running && timed_out(ctx.timing(), &node_run) {
            return super::execution::complete(
                time_out(ctx, &node_run, "Try node timed out").await,
            );
        }
        ...
        match phase.as_str() {
            ...
            _ => super::execution::complete(block_node(ctx, "Try node has invalid phase").await),
        }
    }
}
...
async fn cancel_losing_race_branches<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
    branches: &[String],
    winner: &str,
) -> Result<(), SendableError> {
