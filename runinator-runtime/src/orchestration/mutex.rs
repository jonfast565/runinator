...
pub(super) struct MutexOps<'a, T: RuntimeStore> {
    db: &'a T,
}

impl<'a, T: RuntimeStore> MutexOps<'a, T> {
...
pub(super) async fn enqueue_mutex_wake<T: RuntimeStore>(
    db: &T,
