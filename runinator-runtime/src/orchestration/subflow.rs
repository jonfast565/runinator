...
pub(super) async fn resolve_subflow_id<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
...
/// the runtime drives it. the parent linkage lets a terminal child wake the waiting parent node.
pub(super) async fn create_subflow_run<T: RuntimeStore>(
    ctx: &super::execution::NodeExecutionContext<'_, T>,
    workflow_id: Uuid,
    parameters: Value,
