...
//! `MAX_CALLS_PER_DRIVE` because a program that yields is *supposed* to leave the runtime — a drive
...
impl InvocationOp {
    ...
    async fn drive_running<T: RuntimeStore>(
        &self,
        ctx: &super::execution::NodeExecutionContext<'_, T>,
    ...
    async fn advance<T: RuntimeStore>(
        &self,
        ctx: &super::execution::NodeExecutionContext<'_, T>,
        node_run: &WorkflowNodeRun,
    ...
    async fn dispatch<T: RuntimeStore>(
        &self,
        ctx: &super::execution::NodeExecutionContext<'_, T>,
        node_run: &WorkflowNodeRun,
...
/// ir is that the runtime already knows which single function it wants, so shipping a program plus
