//! graph-runtime behaviour driven directly against an in-memory store.
//!
//! before the runtime's bound was narrowed to `RuntimeStore`, these paths were reachable only from
//! `runinator-ws`'s suite, which boots a real sqlite database. that is why the two bugs re-derived
//! below shipped: nothing could reach a parked node cheaply enough for anyone to write the test.
//!
...
use crate::{DriveOutcome, ReadyNodeDisposition, WorkflowMachine, process_ready_node};
...
async fn a_signal_node_inside_its_timeout_stays_parked() {
    ...
    let cursor_id = store
        .run(parked.workflow_run_id)
        .and_then(|run| run.execution_state.primary_cursor().map(|cursor| cursor.id))
        .expect("parked cursor");

    let outcome = WorkflowMachine::new(&store)
        .drive(parked.workflow_run_id, cursor_id)
        .await
        .expect("second drive");
    assert_eq!(outcome, DriveOutcome::Suspended(crate::Suspension::Signal));
    ...
    let cursor = run
        .execution_state
        .primary_cursor()
        .expect("the parked run has a durable cursor");
    assert!(
        cursor.visit_id.is_some(),
        "the graph interpreter stamps a stable visit before executing the cursor"
    );
    assert_eq!(
        cursor.node_run_id,
        Some(parked.id),
        "a resumed historical cursor is reconciled to its in-flight node record"
    );
}
