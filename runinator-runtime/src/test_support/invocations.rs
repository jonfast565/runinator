...
//! `(invocation, sequence)`. a fake that appended blindly would let a runtime test pass while the
...
impl FakeStore {
    ...
    /// the invocation attached to a node run, if an operation created one.
