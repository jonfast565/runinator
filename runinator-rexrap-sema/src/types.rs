// maps the rexrap `params { }` type expression onto RuninatorType.

use std::collections::{BTreeMap, HashSet};

use runinator_models::types::{RuninatorField, RuninatorType};

use runinator_rexrap_syntax::ast::{TypeDecl, TypeExpr};
...
pub fn resolve_named_types(decls: &[TypeDecl]) -> Result<NamedTypes, RexRapError> {
...
fn type_refs_resolved(ty: &TypeExpr, named: &NamedTypes) -> bool {
    match ty {
        ...
        TypeExpr::Task(inner) => inner
            .as_ref()
            .is_none_or(|inner| type_refs_resolved(inner, named)),
...
pub fn lower_type(type_expr: &TypeExpr) -> Result<RuninatorType, RexRapError> {
...
pub fn lower_type_with(
    ...
    match type_expr {
        ...
        // Task handles are an authoring-time async type. Workflow wire input/output types cannot
        // carry a live handle, so a concrete task annotation contributes its eventual value type;
        // an elided handle is intentionally `any` until binding inference resolves it.
        TypeExpr::Task(Some(inner)) => lower_type_with(inner, named),
        TypeExpr::Task(None) => Ok(RuninatorType::Any),
