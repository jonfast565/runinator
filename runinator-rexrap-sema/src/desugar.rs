...
pub fn desugar(document: &mut Document) -> Result<(), RexRapError> {
...
pub fn flatten_entries(
    entries: &[(String, Expr)],
    aliases: &AliasTable,
) -> Result<Vec<(String, Expr)>, RexRapError> {
...
pub fn collect_aliases(aliases: &[Alias]) -> Result<AliasTable, RexRapError> {
...
fn expand_block(block: &mut Block, aliases: &AliasTable) -> Result<(), RexRapError> {
...
fn expand_cond(cond: &mut Cond, aliases: &AliasTable) -> Result<(), RexRapError> {
    match &mut cond.kind {
...
fn expand_expr(expr: &mut Expr, aliases: &AliasTable) -> Result<(), RexRapError> {
    match &mut expr.kind {
...
fn resolve_alias(
    ...
    let entries = aliases
        .get(name)
        .ok_or_else(|| RexRapError::semantic(span, format!("unknown alias '{name}'")))?;
