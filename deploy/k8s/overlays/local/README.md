...
service API key so the local `runinator-ws` and `runinator-worker` can run unchanged
with `RUNINATOR_AUTH_ENABLED=true`. The waker is broker-only and does not receive service
credentials.
