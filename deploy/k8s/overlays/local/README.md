...
service API key so the local `runinator-ws`, `runinator-worker`, and pack-import job
can all run unchanged with `RUNINATOR_AUTH_ENABLED=true`. The waker is broker-only and
does not receive service credentials.
