...
use runinator_wdl_sema::CompileOptions;
use runinator_wdl_sema::desugar::AliasTable;
use runinator_wdl_sema::types;
...
fn lower_workflow(
    ...
    if !workflow.type_decls.is_empty() {
        ...
        for decl in &workflow.type_decls {
            ...
            decls.insert(
                decl.name.clone(),
                Value::String(runinator_wdl_syntax::format::format_type(&decl.ty)),
            );
        }
    ...
    if let Some(TypeExpr::Struct { fields, .. }) = &workflow.input {
        ...
        for field in fields {
            if type_expr_uses_declared_name(&field.ty, &lowerer.named_types) {
                overrides.insert(
                    field.name.clone(),
                    Value::String(runinator_wdl_syntax::format::format_type(&field.ty)),
                );
            }
        }
    ...
    if !document.functions.is_empty() {
        ...
        for def in &document.functions {
            sigs.insert(
                def.name.clone(),
                Value::String(runinator_wdl_syntax::format::format_fn_signature(def)),
            );
        }
...
pub fn lower_expression_fragment(expr: &Expr, options: &CompileOptions) -> Result<Value, WdlError> {
    let mut lowerer = Lowerer::new();
    lowerer.source_dir = options.source_dir.clone();
    lowerer.lower_expr(expr)
}

pub fn lower_condition_fragment(cond: &Cond, options: &CompileOptions) -> Result<Value, WdlError> {
    let mut lowerer = Lowerer::new();
    lowerer.source_dir = options.source_dir.clone();
...
impl Lowerer {
    ...
    fn record_declared_type(&mut self, id: &str, stmt: &Stmt) -> Result<(), WdlError> {
        ...
        let rendered = runinator_wdl_syntax::format::format_type(type_expr);
        self.declared_types
    ...
    fn lower_value_collector(&mut self, stmt: &Stmt, id: &str, next: &str) -> Result<(), WdlError> {
        ...
        let synthetic = Stmt {
            ...
            compensation: None,
            comments: runinator_wdl_syntax::comments::CommentSet::default(),
        };
    ...
    fn lower_action(
        ...
        let flat = runinator_wdl_sema::desugar::flatten_entries(&action.args, &self.aliases)?;
    ...
    fn lower_subflow(
        ...
        let flat = runinator_wdl_sema::desugar::flatten_entries(&subflow.params, &self.aliases)?;
    ...
    fn lower_approval(
        ...
        let flat = runinator_wdl_sema::desugar::flatten_entries(&approval.metadata, &self.aliases)?;
        for (name, value) in &flat {
            params.insert(name.clone(), self.lower_expr(value)?);
        }
    ...
    fn lower_signal(
        ...
        let flat = runinator_wdl_sema::desugar::flatten_entries(&signal.metadata, &self.aliases)?;
        for (name, value) in &flat {
            params.insert(name.clone(), self.lower_expr(value)?);
        }
