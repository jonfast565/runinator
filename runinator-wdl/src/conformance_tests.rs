//! exhaustive node-kind conformance for the wdl surface.
//!
...
//! fails here until it is either given a fixture or listed in `NO_WDL_SURFACE`.
...
const NO_WDL_SURFACE: &[(WorkflowNodeKind, &str)] = &[];

/// one wdl workflow per node kind, written the way an author would write it.
///
...
fn every_node_kind_is_accounted_for() {
    ...
    for kind in WorkflowNodeKind::ALL {
        ...
        let excused = NO_WDL_SURFACE.iter().any(|(excused, _)| *excused == kind);
        assert!(
            has_fixture ^ excused,
            "{kind:?} needs exactly one of: a wdl fixture in `fixtures()`, or an entry in \
             `NO_WDL_SURFACE` explaining why it has no surface syntax"
        );
    }
}
...
///
/// the editor regenerates its wdl pane via decompile, so a kind whose decompiler emits
