...
The editor surface over [`runinator-wdl`](../runinator-wdl): **completion** and **hover**.
...
```rust
let response = runinator_wdl_ide::complete_source(WdlCompletionRequest {
    source, cursor_byte, providers, settings,
});

let hover = runinator_wdl_ide::hover_source(WdlHoverRequest {
    source, cursor_byte, providers, settings,
});
```
...
| `runinator-ws` | `POST /wdl/complete`, `POST /wdl/hover` |
| `runinator-command-center` | Tauri commands backing the WDL editor pane |

## Boundary

This crate reads the language core through `parse_document`, `ast`, and
[`runinator_wdl::analysis`](../runinator-wdl/src/analysis.rs) — a deliberately short list that
...
```bash
cargo test -p runinator-wdl-ide
```
