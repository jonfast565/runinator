...
impl<T: DurableWorkspaceStore> SharedObjects<T> {
    pub(super) fn record_key(location: &WorkspaceObjectLocation) -> Id {
        Id::sha256(format!("{}:{}:{}", location.pack, location.offset, location.length).as_bytes())
    }

    pub(super) async fn location(&self, id: Id) -> Result<WorkspaceObjectLocation, SendableError> {
    ...
    pub(super) fn read_location_group(
        &self,
        location: &WorkspaceObjectLocation,
    ...
    pub(super) fn read(&self, id: Id) -> storage::Result<Object> {
    ...
    pub(super) async fn read_pack_range(
        &self,
        pack: &str,
        offset: u64,
        length: u64,
