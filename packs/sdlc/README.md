...
- `settings.json` — a settings bundle that seeds every `config.*` value and `secret.*` token the
  workflows reference. Import it once per environment, then edit the values for your org.
...
Seed the store from the bundle (config values are inferred-typed; secrets are stored verbatim):

```bash
runinatorctl settings import packs/sdlc/settings.json
runinatorctl settings list                       # config + secret slots, no values
runinatorctl settings set jira token <api-token> --kind secret
```
...
```bash
runinatorctl wdl compile packs/sdlc/wdl/ticket-work.wdl   # one workflow -> JSON
runinatorctl workflows apply ./packs/sdlc/sdlc.wdlp        # whole pack
```

`runinatorctl workflows apply` also accepts a directory of `.wdl` files (each one workflow) as a pack.
