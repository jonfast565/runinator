...
Import the whole pack in one step. The manifest's `"settings": "settings.json"` entry means
...
Pass `--skip-settings` to import only the workflows. To compile a single workflow to JSON, use
`runinatorctl wdl compile packs/sdlc/wdl/ticket-work.wdl`.

`runinatorctl workflows apply` also accepts a directory of `.wdl` files (each one workflow) as a
pack; a sibling `settings.json` is seeded the same way.
