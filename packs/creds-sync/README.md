...
- `rexrap/update-claude-auth.rrx` — syncs the Claude Code login into the `claude-credentials` Secret.
- `rexrap/update-aws-auth.rrx` — refreshes the AWS SSO cache and syncs it into the `aws-sso-cache` Secret.
