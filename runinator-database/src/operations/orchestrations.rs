    ...
    <B::Db as Database>::QueryResult: RowsAffected,
{
    async fn create_orchestration_binding(
        ...
        let insert = if self.dialect() == SqlDialect::MySql {
            "INSERT INTO orchestration_bindings (id, admission_id, org_id, scope, correlation_key, generation, pipeline_id, pipeline_revision, pipeline_digest, policy, status, resources, budgets, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 'null', '{}', ?, ?) ON DUPLICATE KEY UPDATE id = id"
        } else {
            "INSERT INTO orchestration_bindings (id, admission_id, org_id, scope, correlation_key, generation, pipeline_id, pipeline_revision, pipeline_digest, policy, status, resources, budgets, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', 'null', '{}', ?, ?) ON CONFLICT(admission_id, generation) DO NOTHING"
        };
