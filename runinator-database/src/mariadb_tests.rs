// integration tests against live MariaDB. the assertions are the shared parity body in
...
async fn fresh_db(variable: &str) -> Option<(MariaDb, String, String)> {
    ...
    let pool = MariaDb::new(&db_url).await.unwrap();
...
async fn mariadb_full_lifecycle() {
    ...
    let org = Uuid::new_v4();
    sqlx::query("INSERT INTO organizations (id,name,slug,disabled,max_nodes_json,created_at,updated_at) VALUES (?,'Platform','platform',false,'{}',0,0)")
