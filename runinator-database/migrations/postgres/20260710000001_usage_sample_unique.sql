...
-- sampled_at to the interval boundary and inserts ON CONFLICT DO NOTHING, so any number of ws
-- replicas / background workers sampling the same window converge to one row instead of
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_org_usage_unique
