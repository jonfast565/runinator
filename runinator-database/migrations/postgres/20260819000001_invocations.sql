...
CREATE TABLE IF NOT EXISTS workflow_invocation_calls (
    id UUID PRIMARY KEY,
    invocation_id UUID NOT NULL REFERENCES workflow_invocations(id) ON DELETE CASCADE,
    workflow_run_id UUID NOT NULL REFERENCES workflow_runs(id) ON DELETE CASCADE,
...
ALTER TABLE workflow_node_chunks ADD COLUMN IF NOT EXISTS invocation_call_id UUID NULL REFERENCES workflow_invocation_calls(id) ON DELETE CASCADE;
ALTER TABLE workflow_node_artifacts ADD COLUMN IF NOT EXISTS invocation_call_id UUID NULL REFERENCES workflow_invocation_calls(id) ON DELETE CASCADE;
