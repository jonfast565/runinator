...
fn main() {
    println!("cargo:rerun-if-changed=src/rexrap.pest");

    let grammar = fs::read_to_string("src/rexrap.pest").expect("read RexRap grammar");
    let pairs = parser::parse(parser::Rule::grammar_rules, &grammar).expect("parse rexrap grammar");
    pest_meta::validator::validate_pairs(pairs.clone()).expect("validate rexrap grammar");
    let rules = parser::consume_rules(pairs).expect("consume rexrap grammar");
    let by_name = rules
        .iter()
