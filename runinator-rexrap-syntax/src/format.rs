// renders the parsed rexrap ast back to canonical source. comments captured by `attach_comments`
...
impl Formatter {
    ...
    fn await_node(&self, await_stmt: &AwaitStmt) -> String {
        let mut text = match &await_stmt.target {
            AwaitTarget::Workflow(workflow) => format!("await workflow {}", quote(workflow)),
            AwaitTarget::Task(task) => format!("await {task}"),
        };
...
pub fn format_fn_signature(function: &FunctionDef) -> String {
    let params = function
        .params
        .iter()
        .map(|param| {
...
/// render a `TypeExpr` as rexrap type syntax, preserving any declared type names. shared with lowering
...
pub fn format_type(ty: &TypeExpr) -> String {
    match ty {
        ...
        TypeExpr::Task(None) => "task".into(),
        TypeExpr::Task(Some(inner)) => format!("task[{}]", format_type(inner)),
...
pub(crate) fn step_attributes(modifiers: Option<&Modifiers>) -> Vec<String> {
    ...
    if let Some(profile) = &modifiers.profile {
        out.push(format!("@profile({})", quote(profile)));
    }
