...
pub(super) fn parse_number(text: &str, span: Span) -> Result<ExprKind, RexRapError> {
    if text.contains('.') {
        text.parse::<f64>()
            .map(ExprKind::Float)
            .map_err(|_| RexRapError::syntax(span, format!("invalid number '{text}'")))
    } else {
        text.parse::<i64>()
            .map(ExprKind::Int)
            .map_err(|_| RexRapError::syntax(span, format!("invalid integer '{text}'")))
    }
}

pub(super) fn parse_duration(text: &str, span: Span) -> Result<i64, RexRapError> {
    let (digits, unit) = text.split_at(text.len() - 1);
    let amount = digits
        .parse::<i64>()
        .map_err(|_| RexRapError::syntax(span, format!("invalid duration '{text}'")))?;
...
pub(super) fn parse_i64(text: &str, span: Span) -> Result<i64, RexRapError> {
    text.parse::<i64>()
        .map_err(|_| RexRapError::syntax(span, format!("invalid integer '{text}'")))
}

pub(super) fn parse_optional_count(pair: Pair<Rule>) -> Result<Option<i64>, RexRapError> {
    match pair
        .into_inner()
