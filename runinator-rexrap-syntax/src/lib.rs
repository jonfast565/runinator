// runinator-rexrap-syntax: the RexRap surface syntax — everything between source text and the ast,
...
//
// this is the bottom of the RexRap stack: it knows nothing about diagnostics, the workflow json
// model, or provider metadata, and depends on no other runinator crate but `runinator-models`.
// every other RexRap crate reads a document through `parse_document` and this crate's `ast`.
