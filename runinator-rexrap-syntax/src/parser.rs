// walks pest pairs into the rexrap ast. operator precedence is encoded directly in the
...
fn require_active<'a>(
    ...
    inner: &Pair<Rule>,
) -> Result<&'a mut Workflow, RexRapError> {
...
/// parse a standalone REXRAP condition fragment.
pub fn parse_condition_fragment(src: &str) -> Result<Cond, RexRapError> {
        ...
        match inner.as_rule() {
            Rule::fn_recursive => {
                let depth = inner
                    ...
                    .ok_or_else(|| RexRapError::syntax(span, "@recursive requires max_depth"))?;
...
fn parse_fn_body(pair: Pair<Rule>) -> Result<FnBody, RexRapError> {
...
fn parse_fn_param(pair: Pair<Rule>) -> Result<FnParam, RexRapError> {
    let mut name = String::new();
...
fn parse_secret_decl(pair: Pair<Rule>) -> Result<SecretDecl, RexRapError> {
    ...
    let value =
        value.ok_or_else(|| RexRapError::syntax(span, "secret declaration is missing a value"))?;
    Ok(SecretDecl {
        is_config,
        path,
        value,
...
fn parse_pipeline_trigger(pair: Pair<Rule>) -> Result<PipelineTriggerDecl, RexRapError> {
    ...
    let inner = pair
        .into_inner()
        .next()
        .ok_or_else(|| RexRapError::syntax(span, "empty pipeline trigger"))?;
    match inner.as_rule() {
...
fn parse_pipeline_link(pair: Pair<Rule>) -> Result<PipelineLinkDecl, RexRapError> {
    let span = span_of(&pair);
...
fn parse_pipeline_join(pair: Pair<Rule>) -> Result<PipelineJoinDecl, RexRapError> {
    let span = span_of(&pair);
    let mut target = None;
    let mut mode = None;
    let mut parameters = None;
    ...
    Ok(PipelineJoinDecl {
        target: target
            .ok_or_else(|| RexRapError::syntax(span, "a pipeline join needs a target"))?,
...
fn parse_namespace_block(pair: Pair<Rule>) -> Result<Vec<Workflow>, RexRapError> {
    let mut namespace = None;
    let mut workflows = Vec::new();
    for inner in pair.into_inner() {
        match inner.as_rule() {
            ...
            Rule::version => {
                ...
                version =
                    Some(digits.parse::<SemVer>().map_err(|err| {
                        RexRapError::syntax(span, format!("invalid version: {err}"))
                    })?);
            }
            Rule::returns_decl => {
                let ty = inner
...
fn parse_workflow_decl(
    pair: Pair<Rule>,
) -> Result<(String, Option<SemVer>, Option<TypeExpr>, Span), RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut version = None;
    let mut output = None;
...
fn parse_watch_decl(pair: Pair<Rule>) -> Result<WatchDecl, RexRapError> {
    let mut cond = None;
    ...
    Ok(WatchDecl {
        cond: cond.ok_or_else(|| RexRapError::lower("watch missing condition"))?,
        handler: handler.ok_or_else(|| RexRapError::lower("watch missing handler target"))?,
    })
}
...
fn parse_notify_decl(pair: Pair<Rule>) -> Result<NotifyDecl, RexRapError> {
    let span = span_of(&pair);
    let mut event = None;
    ...
    let event = event.ok_or_else(|| RexRapError::syntax(span, "notify is missing an event"))?;
    let channel =
        channel.ok_or_else(|| RexRapError::syntax(span, "notify is missing a channel"))?;
    let target = target.ok_or_else(|| RexRapError::syntax(span, "notify is missing a target"))?;
...
fn parse_trigger_decl(pair: Pair<Rule>) -> Result<TriggerDecl, RexRapError> {
    let span = span_of(&pair);
    let inner = first_inner(pair)?;
    match inner.as_rule() {
...
fn parse_cron_trigger(pair: Pair<Rule>, span: Span) -> Result<TriggerDecl, RexRapError> {
    ...
    let schedule = schedule
        .ok_or_else(|| RexRapError::syntax(span, "trigger is missing a cron expression"))?;
    Ok(TriggerDecl {
...
fn parse_trigger_catchup(pair: Pair<Rule>, span: Span) -> Result<CatchupDecl, RexRapError> {
...
fn parse_concurrency_decl(pair: Pair<Rule>) -> Result<ConcurrencyDecl, RexRapError> {
    let span = span_of(&pair);
    ...
    let max_concurrent_runs = max_concurrent_runs
        .ok_or_else(|| RexRapError::syntax(span, "concurrency is missing a run limit"))?;
...
fn parse_slot_count(raw: &str, span: Span) -> Result<i64, RexRapError> {
    raw.parse::<i64>()
        .map_err(|_| RexRapError::syntax(span, format!("expected a whole number, got `{raw}`")))
}

fn parse_chained_trigger(pair: Pair<Rule>, span: Span) -> Result<TriggerDecl, RexRapError> {
    let mut event = None;
    let mut target = None;
    let mut params = None;
    let mut enabled = true;
    ...
    let event =
        event.ok_or_else(|| RexRapError::syntax(span, "chained trigger is missing an event"))?;
    let target = target
        .ok_or_else(|| RexRapError::syntax(span, "chained trigger is missing a target workflow"))?;
    Ok(TriggerDecl {
...
fn parse_alias_decl(pair: Pair<Rule>) -> Result<Alias, RexRapError> {
    let span = span_of(&pair);
    let mut inner = pair.into_inner();
    let name = inner
        .next()
        .ok_or_else(|| RexRapError::lower("alias name"))?
        .as_str()
        .to_string();
    let object = inner
        .next()
        .ok_or_else(|| RexRapError::lower("alias body"))?;
...
fn parse_params_block(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
        ...
        match inner.as_rule() {
            ...
            Rule::type_additional => {
                let ty = inner
                    .into_inner()
                    .find(|p| p.as_rule() == Rule::type_expr)
                    .ok_or_else(|| RexRapError::lower("open parameter additional type"))?;
...
fn parse_params_field(pair: Pair<Rule>) -> Result<TypeField, RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut optional = false;
    let mut ty = TypeExpr::Named("any".into());
...
fn parse_type_field(pair: Pair<Rule>) -> Result<TypeField, RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut optional = false;
    let mut ty = TypeExpr::Named("any".into());
...
fn parse_type_expr(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    // type_expr -> type_union
    let union = first_inner(pair)?;
    parse_type_union(union)
}

fn parse_type_union(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
...
fn parse_type_range(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    let span = span_of(&pair);
    let mut inner = pair.into_inner();
    let base = inner
        .next()
        .ok_or_else(|| RexRapError::syntax(span, "range type is missing a base type"))
...
fn parse_type_bound(pair: Pair<Rule>) -> Result<Value, RexRapError> {
    let span = span_of(&pair);
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        ...
        Rule::number if inner.as_str().contains('.') => {
            let value = inner
                .as_str()
                .parse::<f64>()
                .map_err(|_| RexRapError::syntax(span, "invalid range bound"))?;
            Ok(Value::from(value))
        }
        Rule::number => Ok(Value::from(parse_i64(inner.as_str(), span)?)),
        _ => Err(RexRapError::syntax(span, "invalid range bound")),
    }
}

fn parse_type_postfix(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    let mut ty = None;
    let mut suffixes = 0;
    for inner in pair.into_inner() {
        match inner.as_rule() {
    ...
    let mut ty = ty.ok_or_else(|| RexRapError::lower("missing type atom"))?;
    for _ in 0..suffixes {
...
fn parse_type_atom(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        Rule::type_task => Ok(TypeExpr::Task(
            inner
                .into_inner()
                .find(|part| part.as_rule() == Rule::type_expr)
                .map(parse_type_expr)
                .transpose()?
                .map(Box::new),
        )),
        ...
        other => Err(RexRapError::lower(format!(
            "unexpected type atom {other:?}"
        ))),
    }
}

fn parse_type_function(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    ...
    let ret = types
        .pop()
        .ok_or_else(|| RexRapError::lower("function type is missing a return type"))?;
...
fn parse_type_enum(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    let values = pair
        .into_inner()
...
fn parse_enum_lit(pair: Pair<Rule>) -> Result<Value, RexRapError> {
    let span = span_of(&pair);
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        ...
        Rule::number if inner.as_str().contains('.') => {
            let value = inner
                .as_str()
                .parse::<f64>()
                .map_err(|_| RexRapError::syntax(span, "invalid enum number"))?;
            Ok(Value::from(value))
        }
        Rule::number => Ok(Value::from(parse_i64(inner.as_str(), span)?)),
        Rule::boolean => Ok(Value::Bool(inner.as_str() == "true")),
        Rule::null_lit => Ok(Value::Null),
        _ => Err(RexRapError::syntax(span, "invalid enum literal")),
    }
}

fn parse_type_struct(pair: Pair<Rule>) -> Result<TypeExpr, RexRapError> {
    let mut fields = Vec::new();
    let mut additional = None;
    for part in pair.into_inner() {
        match part.as_rule() {
            ...
            Rule::type_additional => {
                let ty = part
                    .into_inner()
                    .find(|p| p.as_rule() == Rule::type_expr)
                    .ok_or_else(|| RexRapError::lower("open struct additional type"))?;
                additional = Some(Box::new(parse_type_expr(ty)?));
            }
            _ => {}
        }
...
fn parse_type_decl(pair: Pair<Rule>) -> Result<TypeDecl, RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut ty = None;
    for inner in pair.into_inner() {
    ...
    let ty = ty.ok_or_else(|| RexRapError::syntax(span, "type declaration is missing a body"))?;
    Ok(TypeDecl {
        name,
        ty,
        span,
...
fn parse_stmt(pair: Pair<Rule>) -> Result<Stmt, RexRapError> {
    ...
    if matches!(kind, StmtKind::Yield(_)) && !transitions.is_empty() {
        return Err(RexRapError::syntax(
            span,
            "`yield` cannot declare transitions",
        ));
    }
    Ok(Stmt {
        span,
...
fn parse_stmt_body(pair: Pair<Rule>) -> Result<StmtKind, RexRapError> {
    let inner = first_inner(pair)?;
    match inner.as_rule() {
...
fn parse_action(pair: Pair<Rule>) -> Result<ActionStmt, RexRapError> {
    ...
    let function = idents
        .pop()
        .ok_or_else(|| RexRapError::lower("action function"))?;
...
fn parse_foreign_do(pair: Pair<Rule>) -> Result<ForeignDo, RexRapError> {
...
fn parse_do_line(pair: Pair<Rule>) -> Result<ComputeLine, RexRapError> {
    ...
    match inner.as_rule() {
        ...
        Rule::compute_if => {
            ...
            let cond = cond.ok_or_else(|| RexRapError::lower("compute if missing condition"))?;
...
fn parse_lambda(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut params = Vec::new();
    let mut body = None;
    for inner in pair.into_inner() {
    ...
    let body = body.ok_or_else(|| RexRapError::lower("lambda missing body"))?;
    Ok(Expr::new(
        ExprKind::Lambda {
            params,
            body: Box::new(body),
...
fn parse_lib_call(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut args = Vec::new();
...
fn parse_call_arg(
    ...
    match inner.as_rule() {
        Rule::named_arg => {
            ...
            let key = parts
                .next()
                .ok_or_else(|| RexRapError::lower("named argument missing name"))?;
            let value = parts
                .next()
                .ok_or_else(|| RexRapError::lower("named argument missing value"))?;
...
fn parse_condition_expr(expr: Expr, span: Span) -> Result<Expr, RexRapError> {
...
fn parse_arg_list(pair: Pair<Rule>) -> Result<Vec<(String, Expr)>, RexRapError> {
    let mut args = Vec::new();
    for arg in pair.into_inner().filter(|p| p.as_rule() == Rule::arg) {
        let entry = arg
            .into_inner()
            .next()
            .ok_or_else(|| RexRapError::lower("arg entry"))?;
        match entry.as_rule() {
            Rule::arg_spread => args.push(parse_spread_entry(entry)?),
            Rule::arg_pair => {
                let mut inner = entry.into_inner();
                let name = inner.next().ok_or_else(|| RexRapError::lower("arg name"))?;
                let value = inner
                    .next()
                    .ok_or_else(|| RexRapError::lower("arg value"))?;
...
fn parse_spread_entry(pair: Pair<Rule>) -> Result<(String, Expr), RexRapError> {
    let span = span_of(&pair);
    let name = pair
        .into_inner()
        .next()
        .ok_or_else(|| RexRapError::lower("spread alias"))?
        .as_str()
        .to_string();
...
fn apply_step_attribute(
    ...
    match name {
        ...
        "workspace" => {
            ...
            modifiers.workspace_affinity = Some(value.clone());
        }
        "idempotent" => {
            let value = named
                .iter()
                ...
                .ok_or_else(|| {
                    RexRapError::syntax(span, "idempotent requires a `key:` expression argument")
                })?;
    ...
    for inner in pair.into_inner() {
        match inner.as_rule() {
            ...
            Rule::subflow_arg => {
                ...
                let key = parts
                    .next()
                    .ok_or_else(|| RexRapError::lower("subflow argument missing name"))?
                    .as_str()
                    .to_string();
                let value_pair = parts
                    .next()
                    .ok_or_else(|| RexRapError::lower("subflow argument missing value"))?;
                match key.as_str() {
                    "params" => {
...
fn expect_bool_expr(expr: Expr, name: &str) -> Result<bool, RexRapError> {
    match expr.kind {
...
fn parse_wait(pair: Pair<Rule>) -> Result<WaitStmt, RexRapError> {
...
fn parse_output(pair: Pair<Rule>) -> Result<OutputStmt, RexRapError> {
    let mut event_type = None;
    let mut data = None;
    for inner in pair.into_inner() {
        match inner.as_rule() {
...
fn parse_output_block(pair: Pair<Rule>) -> Result<OutputStmt, RexRapError> {
    let mut event_type = None;
    let mut data = None;
...
fn parse_input(pair: Pair<Rule>) -> Result<InputStmt, RexRapError> {
    let prompt = pair
        .into_inner()
        .find(|inner| inner.as_rule() == Rule::expr)
        .map(parse_expr)
...
fn parse_gate(pair: Pair<Rule>) -> Result<GateStmt, RexRapError> {
...
fn parse_signal(pair: Pair<Rule>) -> Result<SignalStmt, RexRapError> {
    let mut name = String::new();
...
fn parse_assert(pair: Pair<Rule>) -> Result<AssertStmt, RexRapError> {
    ...
    for line in pair.into_inner() {
        ...
        let cond = cond.ok_or_else(|| RexRapError::lower("assert line missing condition"))?;
        assertions.push((name, cond));
    }
    Ok(AssertStmt { assertions })
}

fn parse_transform(pair: Pair<Rule>) -> Result<TransformStmt, RexRapError> {
    ...
    for line in pair.into_inner() {
        ...
        let value = value.ok_or_else(|| RexRapError::lower("transform line missing value"))?;
        bindings.push((name, value));
    }
    Ok(TransformStmt { bindings })
}

fn parse_audit(pair: Pair<Rule>) -> Result<AuditStmt, RexRapError> {
    let mut action = None;
    let mut actor = None;
    let mut target = None;
    let mut reason = None;
    for field in pair.into_inner() {
        ...
        let value = value.ok_or_else(|| RexRapError::lower("audit field missing value"))?;
        match kw.as_str() {
    ...
    let action = action.ok_or_else(|| RexRapError::lower("audit requires an action"))?;
    Ok(AuditStmt {
        action,
        actor,
        target,
...
fn parse_checkpoint(pair: Pair<Rule>) -> Result<CheckpointStmt, RexRapError> {
    let name = plain_string(first_inner(pair)?)?;
    Ok(CheckpointStmt { name })
}

fn parse_mutex(pair: Pair<Rule>) -> Result<MutexStmt, RexRapError> {
    let mut name = String::new();
    let mut poll_interval = None;
    let mut timeout = None;
    let mut hold = None;
...
fn parse_throttle(pair: Pair<Rule>) -> Result<ThrottleStmt, RexRapError> {
    let mut name = String::new();
...
fn parse_cooldown(pair: Pair<Rule>) -> Result<CooldownStmt, RexRapError> {
    let mut name = String::new();
    let mut window_seconds = 0;
    for inner in pair.into_inner() {
        match inner.as_rule() {
...
fn parse_await(pair: Pair<Rule>) -> Result<AwaitStmt, RexRapError> {
    let mut target = None;
    let mut key = None;
    let mut mode = None;
    let mut timeout = None;
    for inner in pair.into_inner() {
        match inner.as_rule() {
            Rule::await_workflow => {
                target = Some(AwaitTarget::Workflow(plain_string(first_inner(inner)?)?));
            }
            Rule::await_task => {
                target = Some(AwaitTarget::Task(first_inner(inner)?.as_str().to_string()));
            }
    ...
    let target = target.ok_or_else(|| RexRapError::lower("await requires a workflow or task"))?;
    Ok(AwaitStmt {
        target,
        key,
        mode,
...
fn parse_correlate_decl(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let expr = pair
        .into_inner()
        .find(|inner| inner.as_rule() == Rule::expr)
        .ok_or_else(|| RexRapError::lower("correlate requires a key expression"))?;
    parse_expr(expr)
}

fn parse_debounce(pair: Pair<Rule>) -> Result<DebounceStmt, RexRapError> {
    let mut name = String::new();
...
fn parse_collect(pair: Pair<Rule>) -> Result<CollectStmt, RexRapError> {
    let mut name = String::new();
    let mut max = 0;
    let mut timeout = None;
    for inner in pair.into_inner() {
...
fn parse_barrier(pair: Pair<Rule>) -> Result<BarrierStmt, RexRapError> {
    let mut name = String::new();
    let mut count = 0;
    let mut poll_interval = None;
    let mut timeout = None;
...
fn parse_circuit_breaker(pair: Pair<Rule>) -> Result<CircuitBreakerStmt, RexRapError> {
    let mut name = String::new();
    let mut threshold = 0;
...
fn parse_event_source(pair: Pair<Rule>) -> Result<EventSourceStmt, RexRapError> {
...
fn parse_config(pair: Pair<Rule>) -> Result<ConfigStmt, RexRapError> {
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        ...
        other => Err(RexRapError::lower(format!("unexpected config {other:?}"))),
    }
}

fn parse_fail(pair: Pair<Rule>) -> Result<Option<Expr>, RexRapError> {
...
fn parse_if(pair: Pair<Rule>) -> Result<IfStmt, RexRapError> {
        ...
        match inner.as_rule() {
            ...
            Rule::block => {
                ...
                let cond = pending_cond
                    .take()
                    .ok_or_else(|| RexRapError::lower("if arm missing condition"))?;
                arms.push((cond, body));
            }
            Rule::else_if => {
                let mut cond = None;
                ...
                arms.push((
                    cond.ok_or_else(|| RexRapError::lower("else if missing condition"))?,
                    body,
                ));
            }
            Rule::else_block => {
...
fn parse_for(pair: Pair<Rule>) -> Result<ForStmt, RexRapError> {
...
fn parse_while(pair: Pair<Rule>, negate: bool) -> Result<WhileStmt, RexRapError> {
    let mut cond = None;
    let mut limit = None;
    let mut body = Vec::new();
    for inner in pair.into_inner() {
    ...
    Ok(WhileStmt {
        cond: cond.ok_or_else(|| RexRapError::lower("while loop missing condition"))?,
        negate,
        limit,
        body,
    })
...
fn parse_wait_until(pair: Pair<Rule>) -> Result<WhileStmt, RexRapError> {
    ...
    Ok(WhileStmt {
        cond: cond.ok_or_else(|| RexRapError::lower("wait until missing condition"))?,
        negate: true,
        limit: None,
        body: vec![wait],
    })
...
fn parse_map(pair: Pair<Rule>) -> Result<MapStmt, RexRapError> {
    let mut var = String::new();
    let mut items = Expr::new(ExprKind::Null, Span::default());
    let mut concurrency = None;
    let mut body = Vec::new();
...
fn parse_match(pair: Pair<Rule>) -> Result<MatchStmt, RexRapError> {
...
fn parse_match_arm(pair: Pair<Rule>) -> Result<MatchArm, RexRapError> {
    let mut equals = None;
    let mut when = None;
    let mut body = Vec::new();
    for inner in pair.into_inner() {
...
fn parse_toggle(pair: Pair<Rule>) -> Result<MatchStmt, RexRapError> {
    let mut subject = Expr::new(ExprKind::Null, Span::default());
    let mut on = None;
    let mut off = None;
    for inner in pair.into_inner() {
    ...
    let on = on.ok_or_else(|| RexRapError::lower("toggle requires an `on` arm"))?;
    let off = off.ok_or_else(|| RexRapError::lower("toggle requires an `off` arm"))?;
...
fn parse_split(pair: Pair<Rule>) -> Result<MatchStmt, RexRapError> {
    let mut subject = Expr::new(ExprKind::Null, Span::default());
    let mut arms = Vec::new();
    let mut default = None;
    for inner in pair.into_inner() {
...
fn parse_split_arm(pair: Pair<Rule>) -> Result<MatchArm, RexRapError> {
    let mut weight = None;
    let mut body = Vec::new();
    for inner in pair.into_inner() {
        match inner.as_rule() {
    ...
    let weight =
        weight.ok_or_else(|| RexRapError::lower("split arm requires a percentage weight"))?;
    Ok(MatchArm {
        equals: None,
        when: None,
        weight: Some(weight),
...
fn parse_arm_body(pair: Pair<Rule>) -> Result<Block, RexRapError> {
    // arm_body = { block | stmt }
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        Rule::block => parse_block(inner),
        Rule::stmt => Ok(vec![parse_stmt(inner)?]),
        other => Err(RexRapError::lower(format!("unexpected arm body {other:?}"))),
    }
}

fn parse_parallel(pair: Pair<Rule>) -> Result<ParallelStmt, RexRapError> {
        ...
        match inner.as_rule() {
            Rule::branch => {
                ...
                branches.push(ParallelBranch {
                    label,
                    body: body
                        .ok_or_else(|| RexRapError::lower("parallel branch requires a body"))?,
                });
            }
            Rule::join_clause => {
                for part in inner.into_inner() {
...
fn validate_parallel_branches(
    ...
    if selected.is_empty() {
        return Err(RexRapError::lower("parallel join selector cannot be empty"));
    }
...
fn parse_race(pair: Pair<Rule>) -> Result<RaceStmt, RexRapError> {
    let mut branches = Vec::new();
...
fn parse_try(pair: Pair<Rule>) -> Result<TryStmt, RexRapError> {
    let mut body = Vec::new();
    let mut catch = None;
...
fn parse_resume(pair: Pair<Rule>) -> Result<ResumeStmt, RexRapError> {
    let mode = pair
        .into_inner()
...
fn parse_branch_policy(pair: Pair<Rule>) -> Result<BranchPolicy, RexRapError> {
    let span = span_of(&pair);
    match pair.as_str() {
        "all" => Ok(BranchPolicy::All),
        "any" => Ok(BranchPolicy::Any),
        "first_success" => Ok(BranchPolicy::FirstSuccess),
        other => Err(RexRapError::syntax(
            span,
...
fn parse_block(pair: Pair<Rule>) -> Result<Block, RexRapError> {
    pair.into_inner()
...
fn parse_cond(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    parse_cond_or(first_inner(pair)?)
}

fn parse_cond_or(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    let span = span_of(&pair);
    let mut parts = pair
        .into_inner()
...
fn parse_cond_and(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    let span = span_of(&pair);
    let mut parts = pair
        .into_inner()
...
fn parse_cond_unary(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    let span = span_of(&pair);
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        Rule::not_cond => {
        ...
        other => Err(RexRapError::lower(format!(
            "unexpected cond unary {other:?}"
        ))),
    }
}

fn parse_cond_primary(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    let span = span_of(&pair);
    let inner = first_inner(pair)?;
    match inner.as_rule() {
...
fn parse_cond_cmp(pair: Pair<Rule>) -> Result<Cond, RexRapError> {
    let span = span_of(&pair);
    let mut left = None;
    let mut op = None;
    let mut right = None;
    ...
    Ok(Cond::new(
        CondKind::Cmp {
            left: left.ok_or_else(|| RexRapError::lower("comparison missing left"))?,
            op: op.ok_or_else(|| RexRapError::lower("comparison missing operator"))?,
            right: right.ok_or_else(|| RexRapError::lower("comparison missing right"))?,
        },
        span,
    ))
}

fn parse_cmp_op(text: &str, span: Span) -> Result<CmpOp, RexRapError> {
    Ok(match text {
        "==" => CmpOp::Eq,
        "!=" => CmpOp::Ne,
        ">=" => CmpOp::Ge,
...
fn parse_expr(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
...
fn parse_ternary(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    ...
    let cond = cond.ok_or_else(|| RexRapError::lower("empty ternary"))?;
    match (then, els) {
...
fn parse_cast(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    ...
    let expr = expr.ok_or_else(|| RexRapError::lower("empty cast"))?;
    match ty {
        Some(ty) => Ok(Expr::new(
            ExprKind::Cast {
                expr: Box::new(expr),
...
fn parse_compare(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    ...
    let left = left.ok_or_else(|| RexRapError::lower("empty comparison"))?;
    match (op, right) {
...
fn parse_coalesce(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut parts = pair
        .into_inner()
...
fn parse_concat(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut parts = pair
        .into_inner()
...
fn parse_sum(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut acc = None;
    let mut op = None;
    for inner in pair.into_inner() {
    ...
    acc.ok_or_else(|| RexRapError::lower("empty arithmetic sum"))
}

fn parse_term(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut acc = None;
    let mut op = None;
    for inner in pair.into_inner() {
    ...
    acc.ok_or_else(|| RexRapError::lower("empty arithmetic term"))
}

fn parse_unary(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let inner = first_inner(pair)?;
    match inner.as_rule() {
        Rule::neg_expr => {
            let span = span_of(&inner);
        ...
        other => Err(RexRapError::lower(format!("unexpected unary {other:?}"))),
    }
}

// a primary plus a chain of accesses: `.method(args)` (ufcs call), `.key` / `[expr]` (field/index).
fn parse_postfix(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let mut inner = pair.into_inner();
    let primary = inner
        .next()
        .ok_or_else(|| RexRapError::lower("postfix primary"))?;
...
fn apply_access(base: Expr, access: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&access);
    let mut parts = access.into_inner();
    let first = parts
        .next()
        .ok_or_else(|| RexRapError::lower("empty access"))?;
    match first.as_rule() {
        ...
        other => Err(RexRapError::lower(format!("unexpected access {other:?}"))),
    }
}

// the key of a `.key` field access: an identifier is a string key, an integer is an index.
fn path_seg_key(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
...
fn parse_primary(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let inner = first_inner(pair)?;
    let span = span_of(&inner);
...
fn parse_file_call(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let path = plain_string(first_inner(pair)?)?;
    Ok(Expr::new(ExprKind::FileInclude { path }, span))
}

fn parse_dir_call(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut path = None;
    ...
    for inner in pair.into_inner() {
        match inner.as_rule() {
            ...
            Rule::number => {
                ...
                if depth < 1 {
                    return Err(RexRapError::syntax(span, "dir() depth must be at least 1"));
                }
    ...
    let path = path.ok_or_else(|| RexRapError::syntax(span, "dir() missing path"))?;
    Ok(Expr::new(
        ExprKind::DirInclude {
            path,
            recursive,
...
fn parse_inline_call(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut language = None;
...
fn parse_func_call(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let mut name = String::new();
    let mut arg = None;
    for inner in pair.into_inner() {
    ...
    let arg =
        Box::new(arg.ok_or_else(|| RexRapError::syntax(span, "function call missing argument"))?);
...
fn parse_path(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    ...
    if segs.is_empty() {
        return Err(RexRapError::syntax(span, "empty path"));
    }
...
fn parse_call_policy(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let object = pair
        .into_inner()
        .find(|inner| inner.as_rule() == Rule::object)
        .ok_or_else(|| RexRapError::lower("call policy must be an object"))?;
    parse_object(object)
}

fn parse_object(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    Ok(Expr::new(
...
fn parse_object_entries(pair: Pair<Rule>) -> Result<Vec<(String, Expr)>, RexRapError> {
        ...
        .filter(|p| p.as_rule() == Rule::object_entry)
    {
        ...
        match inner.as_rule() {
            ...
            Rule::object_pair => {
                ...
                entries.push((
                    key,
                    value.ok_or_else(|| RexRapError::lower("object value"))?,
                ));
            }
...
fn parse_array(pair: Pair<Rule>) -> Result<Expr, RexRapError> {
    let span = span_of(&pair);
    let items = pair
        .into_inner()
...
fn expect_int(value: Option<&Expr>, label: &str) -> Result<i64, RexRapError> {
    match value.map(|value| &value.kind) {
        Some(ExprKind::Int(int)) => Ok(*int),
        _ => Err(RexRapError::lower(format!("{label} expects an integer"))),
    }
}
...
fn expect_string(value: &Expr, label: &str) -> Result<String, RexRapError> {
    match &value.kind {
        ...
        _ => Err(RexRapError::lower(format!("{label} expects a string"))),
    }
}

fn expect_bool(value: &Expr, label: &str) -> Result<bool, RexRapError> {
    match &value.kind {
        ExprKind::Bool(flag) => Ok(*flag),
        _ => Err(RexRapError::lower(format!("{label} expects a boolean"))),
    }
}

/// accepts either a bare identifier (`failure`) or a literal string (`"failure"`).
fn expect_ident_or_string(value: &Expr, label: &str) -> Result<String, RexRapError> {
    match &value.kind {
...
fn value_to_target(value: &Expr) -> Result<Target, RexRapError> {
    match &value.kind {
        ...
        _ => Err(RexRapError::lower("target must be a label")),
    }
}

fn first_inner(pair: Pair<Rule>) -> Result<Pair<Rule>, RexRapError> {
    pair.into_inner()
        .next()
