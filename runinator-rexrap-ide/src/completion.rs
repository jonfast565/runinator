...
fn construct_completion_items() -> Vec<RexRapCompletionItem> {
    const CONSTRUCTS: &[(&str, &str, &str, &str, bool)] = &[
        ...
        (
            "pipeline workflow",
            "keyword",
            "pipeline member workflow",
            "workflow \"${workflow}\" on_failure ${mode}",
            true,
        ),
        (
            "pipeline link",
            "keyword",
            "pipeline member dependency",
            "\"${from}\" -> \"${to}\" on ${selector}",
            true,
        ),
        (
    ...
    items.extend(
        PIPELINE_ORCHESTRATION_KEYWORDS
            .iter()
            .map(|keyword| RexRapCompletionItem {
                label: (*keyword).into(),
                kind: "keyword".into(),
