...
//!
//! these moved out of `runinator-rexrap`'s suite with the modules they cover: they assert what an
...
fn hover_at(src: &str, marker: &str) -> RexRapHoverResponse {
    let cursor = src.find(marker).expect("marker");
    let source = src.replacen(marker, "", 1);
    hover_source(RexRapHoverRequest {
        source,
        cursor_byte: cursor,
...
fn completion_labels_with_providers(
    ...
    let cursor = src.find(marker).expect("marker");
    let source = src.replacen(marker, "", 1);
    complete_source(RexRapCompletionRequest {
        source,
        cursor_byte: cursor,
        providers,
        settings: Vec::new(),
...
fn setting_completion(src: &str, marker: &str) -> RexRapCompletionResponse {
