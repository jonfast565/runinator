...
export function renameObjectKey(record: JsonRecord, previousKey: string, nextKey: string): RenameObjectKeyResult {
  const trimmed = nextKey.trim();
  if (!trimmed) return { value: record, error: "Key is required" };
  if (trimmed !== previousKey && Object.prototype.hasOwnProperty.call(record, trimmed)) {
    return { value: record, error: "Key already exists" };
  }
  if (trimmed === previousKey) return { value: record, error: "" };
