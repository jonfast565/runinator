...
describe("WDL expression conversion", () => {
  it("renders lowered references and operators as WDL surface expressions", () => {
    ...
    expect(expressionJsonToWdl({ "$concat": ["ticket ", { "$ref": { params: ["ticket_id"] } }] })).toBe("\"ticket \" ++ params.ticket_id");
  ...
  it("parses WDL surface expressions back into lowered JSON values", () => {
    ...
    expect(parseWdlExpression("\"ticket \" ++ params.ticket_id")).toEqual({ "$concat": ["ticket ", { "$ref": { params: ["ticket_id"] } }] });
    expect(parseWdlExpression("input.ticket_id")).toEqual({ "$ref": { node: "input", output: ["ticket_id"] } });
