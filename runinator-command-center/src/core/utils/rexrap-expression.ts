...
export function expressionJsonToRexRap(value: unknown): string {
  if (value === null) {
    return "null";
  }
  ...
  if (typeof value === "string") {
    return secretRefToRexRap(value) ?? quote(value);
  }

  if (Array.isArray(value)) {
    return `[${value.map(expressionJsonToRexRap).join(", ")}]`;
  }
  ...
  if (keys.length === 1) {
    if (isRecord(value.$ref)) {
      return refToRexRap(value.$ref);
    }

    if (Array.isArray(value.$concat)) {
      return joinOperator(value.$concat, " ++ ");
    }

    if (Array.isArray(value.$coalesce)) {
      return joinOperator(value.$coalesce, " ?? ");
    }

    if (Object.prototype.hasOwnProperty.call(value, "$to_string")) {
      return `string(${expressionJsonToRexRap(value.$to_string)})`;
    }

    if (Object.prototype.hasOwnProperty.call(value, "$to_json_string")) {
      return `json(${expressionJsonToRexRap(value.$to_json_string)})`;
    }
  }

  const entries = Object.entries(value).map(
    ([key, nested]) => `${objectKey(key)}: ${expressionJsonToRexRap(nested)}`,
  );
  return `{ ${entries.join(", ")} }`;
}

export function parseRexRapExpression(source: string): JsonValue {
...
export function isWorkflowExpressionValue(value: unknown): value is JsonRecord {
  return (
    isRecord(value) &&
    expressionKeys.some((key) => Object.prototype.hasOwnProperty.call(value, key))
  );
}

function joinOperator(items: unknown[], operator: string): string {
  return items.map((item) => wrapBinaryPart(expressionJsonToRexRap(item))).join(operator);
}
...
function refToRexRap(ref: JsonRecord): string {
  if (Array.isArray(ref.params)) {
    return appendPath("params", ref.params);
  }

  if (Array.isArray(ref.prev)) {
    return appendPath("prev", ref.prev);
  }

  if (Array.isArray(ref.workflow)) {
    return appendPath("run", ref.workflow);
  }

  if (Array.isArray(ref.config)) {
    return appendPath("config", ref.config);
  }

  if (typeof ref.node === "string" && Array.isArray(ref.output)) {
    return appendPath(ref.node, ref.output);
  }
...
function objectLiteral(record: JsonRecord): string {
  const entries = Object.entries(record).map(
    ([key, value]) => `${objectKey(key)}: ${expressionJsonToRexRap(value)}`,
  );
  return `{ ${entries.join(", ")} }`;
}
...
function secretRefToRexRap(value: string): string | null {
  ...
  if (
    !scope ||
    name.length === 0 ||
    ![scope, ...name].every((part) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(part))
  ) {
    return null;
  }
...
class Parser {
  ...
  private parseIdentPrimary(): JsonValue {
    ...
    while (this.match("punct", ".")) {
      path.push(this.expectPathSegment());
    }
  ...
  private parseObject(): JsonValue {
    ...
    while (!this.match("punct", "}")) {
      ...
      if (keyToken.kind === "string") {
        key = JSON.parse(this.expect("string").text) as string;
      } else {
  ...
  private expectPathSegment(): string {
    ...
    if (token.kind !== "ident" && token.kind !== "number") {
      throw new Error("Expected path segment");
    }
...
function lowerPath(path: string[]): JsonValue {
  ...
  if (head === "params" || head === "prev" || head === "config") {
    return { $ref: { [head]: pathSegments(rest) } };
  }

  if (head === "run" || head === "workflow") {
    return { $ref: { workflow: pathSegments(rest) } };
  }

  if (head === "secret" && rest.length >= 2) {
    return `secret://${rest[0]}/${rest.slice(1).join("/")}`;
  }
