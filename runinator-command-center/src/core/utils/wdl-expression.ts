...
export function expressionJsonToWdl(value: unknown): string {
  ...
  if (typeof value === "string") {
    return secretRefToWdl(value) ?? quote(value);
  }
  ...
  if (keys.length === 1) {
    ...
    if (Object.prototype.hasOwnProperty.call(value, "$to_string")) {
      return `string(${expressionJsonToWdl(value.$to_string)})`;
    }

    if (Object.prototype.hasOwnProperty.call(value, "$to_json_string")) {
      return `json(${expressionJsonToWdl(value.$to_json_string)})`;
    }
  }

  const entries = Object.entries(value).map(
    ([key, nested]) => `${objectKey(key)}: ${expressionJsonToWdl(nested)}`,
  );
  return `{ ${entries.join(", ")} }`;
}
...
function objectLiteral(record: JsonRecord): string {
  const entries = Object.entries(record).map(
    ([key, value]) => `${objectKey(key)}: ${expressionJsonToWdl(value)}`,
  );
  return `{ ${entries.join(", ")} }`;
}
