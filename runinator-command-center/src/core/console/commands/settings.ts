import { defaultApi, type ConsoleSettingsApi } from "../../api/ports/console-settings";
...
export function createSettingsCommands(api: ConsoleSettingsApi = defaultApi) {
  const settingsCommands: ConsoleCommand[] = [
    {
      path: ["settings", "list"],
      ...
      run: async ({ flags, json: raw, print }) => {
        ...
        let entries = await api.fetchCredentials();
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        const detail = await api.fetchCredential(
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        const name = requiredArg(args, 1, "name");
        const kind = settingKind(flags);
        await api.deleteCredential(scope, name, kind);
...
export const { settingsCommands } = createSettingsCommands();
