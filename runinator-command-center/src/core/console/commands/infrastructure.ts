import { defaultApi, type ConsoleInfrastructureApi } from "../../api/ports/console-infrastructure";
...
export function createInfrastructureCommands(api: ConsoleInfrastructureApi = defaultApi) {
  const nodeCommands: ConsoleCommand[] = [
    {
      path: ["nodes", "list"],
      usage: "nodes list",
      summary: "list provisioning backends and current node group sizing",
      run: async ({ json: raw, print }) => {
        const [backends, groups] = await Promise.all([api.fetchNodeBackends(), api.fetchNodes()]);
    ...
    },
    {
      ...
      run: async ({ flags, print }) => {
        ...
        const groups = await api.fetchNodes();
        const current =
          groups.find((group) => group.backend === backend && group.kind === kind)?.desired ?? 0;
        const group = await api.scaleNodes({
          backend,
          kind,
    ...
    },
    {
      ...
      run: async ({ flags, print }) => {
        const response = await api.stopNode({
          backend: requiredFlag(flags, "backend"),
  ...
  const replicaCommands: ConsoleCommand[] = [
    {
      path: ["replicas", "list"],
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        ...
        const { replicas } = await api.fetchReplicas();
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const registrations = await api.fetchReplicaProviders(requiredArg(args, 0, "replica id"));
    ...
    },
    {
      ...
      run: async ({ args, flags, json: raw, print }) => {
        const series = await api.fetchReplicaSamples(
  ...
  const orgCommands: ConsoleCommand[] = [
    {
      path: ["orgs", "list"],
      usage: "orgs list",
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        const org = await api.createOrg(requiredArg(args, 0, "name"));
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const nodes = await api.fetchOrgNodes(requiredArg(args, 0, "org id"));
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        const group = await api.scaleOrgNodes(requiredArg(args, 0, "org id"), {
          backend: requiredFlag(flags, "backend"),
          kind: requiredFlag(flags, "kind"),
          desired: Number(requiredFlag(flags, "desired")),
        });
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const usage = await api.fetchOrgUsage(requiredArg(args, 0, "org id"));
  ...
  const agentCommands: ConsoleCommand[] = [
    ...
    ),
    {
      ...
      run: async ({ args, flags, json: raw, print }) => {
        const directives = await api.listAgentDirectives(
          requiredArg(args, 0, "replica id"),
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const replica = await api.kickReplica(requiredArg(args, 0, "replica id"));
    ...
    },
    {
      ...
      booleans: ["permanent"],
      run: async ({ flags, print }) => {
        const response = await api.createAgentEnrollmentToken({
    ...
    },
    {
      ...
      run: async ({ json: raw, print }) => {
        const tokens = await api.listAgentEnrollmentTokens();
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        await api.revokeAgentEnrollmentToken(tokenId);
    ...
    },
    {
      ...
      run: async ({ json: raw, print }) => {
        const machines = await api.listAgentMachines();
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        await api.invalidateAgentMachine(machineId);
  ...
  function directive(
    ...
    return {
      ...
      summary,
      run: async ({ args, flags, print }) => {
        const record = await api.createAgentDirective(
          requiredArg(args, 0, "replica id"),
          kind(flags),
        );
        print(json(record));
  ...
  return { nodeCommands, replicaCommands, orgCommands, agentCommands };
}

export const { nodeCommands, replicaCommands, orgCommands, agentCommands } =
  createInfrastructureCommands();
