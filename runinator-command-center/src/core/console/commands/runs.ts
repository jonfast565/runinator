import { defaultApi, type ConsoleRunsApi } from "../../api/ports/console-runs";
...
import { createConsoleLookup } from "../lookup";
...
export function createRunsCommands(api: ConsoleRunsApi = defaultApi) {
  const { isUuid, resolveWorkflowId } = createConsoleLookup(api);
  const OPEN_STATUSES = [
    "queued",
    "running",
    "paused",
  ...
  const runCommands: ConsoleCommand[] = [
    {
      path: ["runs", "list"],
      ...
      run: async ({ flags, json: raw, print }) => {
        ...
        const workflowId = workflow
          ...
          : undefined;
        let runs = await api.fetchWorkflowRuns(workflowId);
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const detail = await api.fetchWorkflowRun(requiredArg(args, 0, "run id"));
    ...
    },
    {
      ...
      run: async ({ args, print, signal }) => {
        ...
        while (!signal.aborted) {
          const detail = await api.fetchWorkflowRun(runId);
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const output = await api.fetchWorkflowEffectOutput(requiredArg(args, 0, "effect id"));
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const artifacts = await api.fetchWorkflowRunArtifacts(requiredArg(args, 0, "run id"));
    ...
    control("pause", "pause a workflow run", api.pauseWorkflowRun),
    control("resume", "resume a workflow run", api.resumeWorkflowRun),
    control("cancel", "cancel a workflow run", api.cancelWorkflowRun),
    {
      path: ["runs", "replay"],
      usage: "runs replay <run-id> [--from-step STEP]",
      summary: "replay a workflow run",
      run: async ({ args, flags, print }) => {
        const created = await api.replayWorkflowRun(requiredArg(args, 0, "run id"), {
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        print(
          json(await api.fetchReplayPlan(requiredArg(args, 0, "run id"), flag(flags, "from-step"))),
        );
      },
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        await api.renameWorkflowRun(runId, name === "" ? null : name);
...
export const { runCommands } = createRunsCommands();
