import { defaultApi, type ConsoleTriggersApi } from "../../api/ports/console-triggers";
...
export function createTriggersCommands(api: ConsoleTriggersApi = defaultApi) {
  const { resolveWorkflowId } = createConsoleLookup(api);
  const triggerCommands: ConsoleCommand[] = [
    {
      path: ["triggers", "list"],
      ...
      run: async ({ args, json: raw, print }) => {
        ...
        const triggers = await api.fetchWorkflowTriggers(workflowId);
    ...
    },
    {
      ...
      run: async ({ json: raw, print }) => {
        const triggers = await api.fetchDueTriggers();
        print(raw ? json(triggers) : triggerTable(triggers));
      },
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        const created = await api.createTriggerRun(
    ...
    },
    {
      ...
      booleans: ["dry-run"],
      run: async ({ args, flags, print }) => {
        const response = await api.backfillWorkflowTrigger(requiredArg(args, 0, "trigger id"), {
  ...
  const freezeCommands: ConsoleCommand[] = [
    {
      path: ["freeze", "list"],
      ...
      run: async ({ flags, json: raw, print }) => {
        const windows = await api.fetchFreezeWindows(flags.active !== undefined);
    ...
    },
    {
      path: ["freeze", "create"],
      usage:
        "freeze create <name> --from RFC3339 --to RFC3339 [--workflow-id ID] [--org-id ID] [--reason TEXT]",
      summary: "suspend trigger firing over a time range",
      run: async ({ args, flags, print }) => {
        const created = await api.createFreezeWindow({
  ...
  return { triggerCommands, freezeCommands };
}

export const { triggerCommands, freezeCommands } = createTriggersCommands();
