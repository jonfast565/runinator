import { defaultApi, type ConsoleRexrapApi } from "../../api/ports/console-rexrap";
...
export function createRexrapCommands(api: ConsoleRexrapApi = defaultApi) {
  const { resolveWorkflow } = createConsoleLookup(api);
  const rexrapCommands: ConsoleCommand[] = [
    {
      path: ["rexrap", "check"],
      usage: "rexrap check [--cell N]",
      summary: "parse, lower, and validate a cell, printing any diagnostics",
      run: async (context) => {
        const diagnostics = await api.analyzeRexRap(source(context));
    ...
    },
    {
      ...
      run: async (context) => {
        context.print(text(await api.formatRexRap(source(context))));
      },
    },
    {
      path: ["rexrap", "compile"],
      ...
      run: async (context) => {
        const workflow = await api.compileRexRap(source(context), false);
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        print(text(await api.decompileToRexRap(workflow)));
      },
    },
  ];
  ...
  return { rexrapCommands };
}

export const { rexrapCommands } = createRexrapCommands();
