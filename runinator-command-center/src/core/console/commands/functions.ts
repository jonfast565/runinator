import { defaultApi, type ConsoleFunctionsApi } from "../../api/ports/console-functions";
...
export function createFunctionsCommands(api: ConsoleFunctionsApi = defaultApi) {
  const functionCommands: ConsoleCommand[] = [
    {
      path: ["functions", "list"],
      usage: "functions list",
      summary: "list published packages",
      run: async ({ json: raw, print }) => {
        const packages = await api.fetchFunctionPackages();
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        const detail = await api.fetchFunctionPackage(requiredArg(args, 0, "package"));
    ...
    },
    {
      ...
      run: async ({ json: raw, print }) => {
        const catalog = await api.fetchFunctionCatalog();
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        await api.setFunctionAlias(
          packageName,
          alias,
          numberFlag(flags, "version"),
          flag(flags, "from"),
        );
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        const packageName = requiredArg(args, 0, "package");
        const alias = requiredArg(args, 1, "alias");
        await api.deleteFunctionAlias(packageName, alias);
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        const packageName = requiredArg(args, 0, "package");
        await api.deleteFunctionPackage(packageName);
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        const packageName = requiredArg(args, 0, "package");
        await api.restoreFunctionPackage(packageName);
...
export const { functionCommands } = createFunctionsCommands();
