import { defaultApi, type ConsoleOperationsApi } from "../../api/ports/console-operations";
...
export function createOperationsCommands(api: ConsoleOperationsApi = defaultApi) {
  const OPEN_STATUSES = [
    "queued",
    "running",
    "paused",
  ...
  const operationCommands: ConsoleCommand[] = [
    {
      path: ["status"],
      usage: "status",
      ...
      run: async ({ json: raw, print }) => {
        ...
        const supervisor = await api.fetchSupervisorStatus().catch(() => null);
    ...
    },
    {
      ...
      run: async ({ flags, json: raw, print }) => {
        let approvals = await api.fetchApprovals(flag(flags, "workflow-run-id"));
    ...
    resolution("approve", api.approveApproval),
    resolution("reject", api.rejectApproval),
    {
      path: ["providers", "list"],
    ...
    },
    {
      ...
      run: async ({ args, json: raw, print }) => {
        ...
        const provider = (await api.fetchProviders()).find((candidate) => candidate.name === name);
    ...
    },
    {
      ...
      run: async ({ flags, json: raw, print }) => {
        ...
        const artifacts = (await api.fetchWorkflowEffectOutput(effect))
...
export const { operationCommands } = createOperationsCommands();
