import { defaultApi, type ConsoleSessionApi } from "../../api/ports/console-session";
...
export function createSessionCommands(api: ConsoleSessionApi = defaultApi) {
  const { resolvePipeline, resolveWorkflowId } = createConsoleLookup(api);
  const sessionCommands: ConsoleCommand[] = [
    {
      path: ["sessions"],
      usage: "sessions",
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        const created = await api.createPipelineRun(pipeline.id, parameters(args, flags));
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        const attempt = await api.retryPipelineMember(runId, memberKey, parameters(args, flags));
