import { defaultApi, type ConsoleWorkflowsApi } from "../../api/ports/console-workflows";
...
export function createWorkflowsCommands(api: ConsoleWorkflowsApi = defaultApi) {
  const { resolveWorkflow, resolveWorkflowId } = createConsoleLookup(api);
  const workflowCommands: ConsoleCommand[] = [
    ...
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        const created = await api.createWorkflowRun(workflowId, {
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        print(json(await api.exportWorkflowBundle(workflowId)));
      },
    },
    {
      ...
      run: async ({ args, flags, json: raw, print }) => {
        ...
        const revisions = await api.fetchWorkflowRevisions(workflowId, limit);
    ...
    },
    {
      ...
      run: async ({ args, print }) => {
        ...
        print(json(await api.fetchWorkflowRevision(workflowId, revision)));
      },
    },
    {
      ...
      run: async ({ args, print }) => {
        const workflowId = await resolveWorkflowId(requiredArg(args, 0, "workflow"));
        const revision = Number(requiredArg(args, 1, "revision"));
        const restored = await api.restoreWorkflowRevision(workflowId, revision);
        print(
          done(`restored revision ${String(revision)} as ${restored.name} v${restored.version}`),
        );
      },
    },
    {
      ...
      run: async ({ args, flags, print }) => {
        ...
        const duplicate = await api.duplicateWorkflow(workflowId, bump);
...
export const { workflowCommands } = createWorkflowsCommands();
