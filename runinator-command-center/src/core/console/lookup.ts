import { defaultApi, type ConsoleLookupApi } from "../api/ports/console-lookup";
...
export function createConsoleLookup(api: ConsoleLookupApi = defaultApi) {
  ...
  async function resolveWorkflow(reference: string): Promise<WorkflowDefinition> {
  ...
  async function resolveWorkflowId(reference: string): Promise<string> {
  ...
  async function resolvePipeline(reference: string): Promise<Pipeline> {
    ...
    const found = pipelines.find(
      (pipeline) =>
        ...
        (pipeline.namespace != null &&
          pipeline.key != null &&
          pipelinePath(pipeline) === reference),
    );
  ...
  return { isUuid, resolveWorkflow, resolveWorkflowId, resolvePipeline };
}

export const { isUuid, resolveWorkflow, resolveWorkflowId, resolvePipeline } =
  createConsoleLookup();
