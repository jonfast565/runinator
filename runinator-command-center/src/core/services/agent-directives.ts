import { createAgentDirective, kickReplica, listAgentDirectives } from "../api/commandCenterApi";
  ...
  return {
    issue(replicaId: string, kind: AgentDirectiveKind) {
      return app.runOperation("Issuing agent directive", () =>
        createAgentDirective(replicaId, kind),
      );
    },
    ...
    kick(replicaId: string) {
      return app.runOperation("Kicking replica", () => kickReplica(replicaId));
    },
  };
}
