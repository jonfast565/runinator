...
export const useResourcesStore = defineStore("resources", () => {
  ...
  async function refreshResources() {
    resourceRecords.value = await app
      .runOperation("Refreshing resources", () =>
        fetchResourceRecords(selectedResourceEndpoint.value),
      )
      .catch(() => []);
  ...
  async function findWorkflowApprovalId(
    workflowRunId: string,
    nodeId: string,
    nodeRun: WorkflowNodeRun,
  ): Promise<string | null> {
    ...
    const approvals = await app.runOperation("Loading workflow approvals", () =>
  ...
  async function deleteSelected() {
    ...
    if (selectedResourceEndpoint.value !== "automation_events") {
      return;
    }
