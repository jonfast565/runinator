...
export function jsonCompletionSource(
  ...
  return (context: CompletionContext): CompletionResult | null => {
    ...
    if (
      objectNode &&
      (quoteMatch || context.explicit) &&
      (quoteMatch
        ? isObjectKeyContext(doc, quoteMatch.from)
        : !isValueContext(doc, context.pos, true, arrayNode != null))
    ) {
      const localKeys = collectObjectKeys(objectNode, doc);
      const globalKeys = collectPropertyNames(syntaxTree(state).topNode, doc);
      const prefix = quoteMatch ? doc.sliceString(quoteMatch.from + 1, context.pos) : "";
      const options = toCompletionOptions(
        mergeUnique(localKeys, hints, globalKeys),
        prefix,
        "property",
      );
      ...
      if (options.length || context.explicit) {
        return {
          from: quoteMatch ? quoteMatch.from + 1 : context.pos,
          options,
          validFor: /^[^"]*$/,
        };
      }
    ...
    if (
      isValueContext(doc, context.pos, objectNode != null, arrayNode != null) ||
      context.explicit
    ) {
...
function findAncestor(
  node: JsonSyntaxNode | null | undefined,
  name: string,
): JsonSyntaxNode | null {
...
function isValueContext(
  ...
  if (!previous) {
    return insideObject || insideArray;
  }
...
function previousMeaningfulChar(
  doc: { sliceString(from: number, to: number): string },
  pos: number,
): string {
...
function mergeUnique(...groups: (string[] | undefined)[]): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
