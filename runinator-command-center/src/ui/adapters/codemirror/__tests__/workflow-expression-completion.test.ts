...
import {
  isWorkflowExpressionValue,
  workflowExpressionCompletionSource,
...
describe("workflow expression detection", () => {
  it("recognizes every expression operator emitted by WDL/runtime expressions", () => {
    for (const key of [
      ...
      expect(
        isWorkflowExpressionValue({ [key]: key === "$concat" || key === "$coalesce" ? [] : null }),
      ).toBe(true);
    }
  });
  ...
  it("offers saveable literal and helper completions in expression editors", async () => {
    ...
    const result = await workflowExpressionCompletionSource(() => undefined)(
