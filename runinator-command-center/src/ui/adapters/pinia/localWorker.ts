...
export const useLocalWorkerStore = defineStore("localWorker", () => {
  ...
  async function refresh() {
    ...
    try {
      ...
      error.value = errorMessage(err) || "Failed to read local worker status";
    }
  }
  ...
  async function start(config: LocalWorkerConfig) {
    ...
    try {
      ...
      error.value = errorMessage(err) || "Failed to start local worker";
    } finally {
      busy.value = false;
    }
  }
  ...
  async function stop() {
    ...
    try {
      ...
      error.value = errorMessage(err) || "Failed to stop local worker";
    } finally {
      busy.value = false;
    }
  }
