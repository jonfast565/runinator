  ...
  <div class="min-h-0 overflow-hidden">
    <section class="grid min-h-0 min-w-0 content-start gap-3 overflow-hidden">
      ...
      <DataTable v-else>
        <table>
          <thead>
            <tr><th>Username</th><th>Email</th><th>Status</th><th>Role</th></tr>
          </thead>
          <tbody>
            <tr
    ...
    <div v-if="modalOpen" class="modal-backdrop" @click.self="closeModal">
      <form class="modal w-full max-w-[860px]" @submit.prevent="save">
        ...
        <div class="form-grid">
          ...
          <label><span>Email</span><input v-model="permissions.userDraft.email" type="email" autocomplete="off" /></label>
          <label>
        ...
        <section class="grid gap-2.5">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex min-w-0 items-center gap-1.5">
              <select v-model="teamId" class="min-w-[140px]" :disabled="!permissions.selectedUser || availableTeams.length === 0">
                <option value="">Add team</option>
                <option v-for="team in availableTeams" :key="String(team.id)" :value="String(team.id)">{{ team.name }}</option>
              </select>
              <button class="btn btn-sm" type="button" :disabled="!teamId" @click="assignTeam">Add</button>
            </div>
          </div>
          <div v-if="permissions.userTeams.length" class="flex flex-wrap gap-1.5">
            <span v-for="team in permissions.userTeams" :key="String(team.id)" class="inline-flex items-center gap-1.5 rounded-pill border border-border bg-surface-subtle px-2 py-1 text-xs">
              {{ team.name }}
              <button type="button" class="cursor-pointer border-0 bg-transparent p-0 text-sm leading-none text-fg-muted" @click="permissions.removeSelectedUserFromTeam(String(team.id))">×</button>
            </span>
          </div>
          <div v-else class="min-h-0 py-2 text-left text-fg-muted">No teams assigned.</div>
        </section>
        <div class="modal-actions">
          <button class="btn btn-danger" type="button" :disabled="!permissions.selectedUser || isLastEnabledAdmin" @click="confirmDelete">
