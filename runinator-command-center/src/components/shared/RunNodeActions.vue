<template>
  <div class="run-node-actions">
    <button type="button" class="btn btn-sm" :disabled="busy" title="Re-run the whole workflow" @click="emitAction('replay-run')">
    ...
    <button v-if="showEditorActions" type="button" class="btn btn-sm" :disabled="busy" title="Open this step in the editor" @click="emitAction('open-editor')">
      <Icon name="edit" :size="13" />
      <span>Step editor</span>
    </button>
    <button v-if="showEditorActions" type="button" class="btn btn-sm" :disabled="busy" title="Open the provider metadata for this step" @click="emitAction('open-provider')">
...
function copyTitle(kind: "input" | "output"): string {
  return kind === "input" ? "Copy this node's input parameters" : "Copy this node's output (or error message)";
}
...
function errorOrOutput(): string {
  if (isFailed.value && props.node.message) return props.node.message;
  const output = props.node.output_json;
  if (output === undefined || output === null) return props.node.message ?? "";
